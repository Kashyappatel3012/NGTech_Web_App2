...
# Create blueprint
change_management_evidence_bp = Blueprint('change_management_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '44.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '44'
                if prefix == '44':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '44' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '44'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for Change Management Evidence"""
    mapping = {}
...
@change_management_evidence_bp.route('/test_change_management_evidence_route')
def test_change_management_evidence_route():
    return "Change Management Evidence route is working!"

@change_management_evidence_bp.route('/process_change_management_evidence', methods=['POST'])
@login_required
def process_change_management_evidence():
    print("="*50)
    print("CHANGE MANAGEMENT EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR CHANGE MANAGEMENT EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING CHANGE MANAGEMENT EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "Change Management Review.xlsx"

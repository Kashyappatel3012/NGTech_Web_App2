...
# Create blueprint
mail_messaging_evidence_bp = Blueprint('mail_messaging_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '11.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '11'
                if prefix == '11':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '11' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '11'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for Mail and Messaging Evidence"""
    mapping = {}
    ...
    # Decimal 3 -> J4, K4, I4
    # .........
    # Decimal 40 -> J41, K41, I41
    
    # Create mappings for decimal numbers 1 to 40
    for decimal_num in range(1, 41):  # 1 to 40 inclusive
...
@mail_messaging_evidence_bp.route('/test_mail_messaging_evidence_route')
def test_mail_messaging_evidence_route():
    return "Mail and Messaging Evidence route is working!"

@mail_messaging_evidence_bp.route('/process_mail_messaging_evidence', methods=['POST'])
@login_required
def process_mail_messaging_evidence():
    print("="*50)
    print("MAIL AND MESSAGING EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR MAIL AND MESSAGING EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING MAIL AND MESSAGING EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "Mail and Messaging Review.xlsx"

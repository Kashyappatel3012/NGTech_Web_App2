...
# Create blueprint
core_switch_evidence_bp = Blueprint('core_switch_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '5.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '5'
                if prefix == '5':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '5' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '5'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for Core Switch Evidence"""
    mapping = {}
...
@core_switch_evidence_bp.route('/test_core_switch_evidence_route')
def test_core_switch_evidence_route():
    return "Core Switch Evidence route is working!"

@core_switch_evidence_bp.route('/process_core_switch_evidence', methods=['POST'])
@login_required
def process_core_switch_evidence():
    print("="*50)
    print("CORE SWITCH EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR CORE SWITCH EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING CORE SWITCH EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "Core Switch Review.xlsx"

...
# Create blueprint
amc_evidence_bp = Blueprint('amc_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '21.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '21'
                if prefix == '21':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '21' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '21'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for AMC Evidence"""
    mapping = {}
...
@amc_evidence_bp.route('/test_amc_evidence_route')
def test_amc_evidence_route():
    return "AMC Evidence route is working!"

@amc_evidence_bp.route('/process_amc_evidence', methods=['POST'])
@login_required
def process_amc_evidence():
    print("="*50)
    print("AMC EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR AMC EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING AMC EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "AMC Review.xlsx"

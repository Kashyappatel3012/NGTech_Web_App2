...
# Create blueprint
network_monitoring_tool_evidence_bp = Blueprint('network_monitoring_tool_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '37.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '37'
                if prefix == '37':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '37' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '37'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for Network Monitor Tool Evidence"""
    mapping = {}
    ...
    # Create mappings for decimal numbers 1 to 2
    for decimal_num in range(1, 3):  # 1 to 2 inclusive
        # For decimal x, row number = x + 1
        # Decimal 1 -> 1 + 1 = 2, Decimal 2 -> 2 + 1 = 3
...
@network_monitoring_tool_evidence_bp.route('/test_network_monitoring_tool_evidence_route')
def test_network_monitoring_tool_evidence_route():
    return "Network Monitor Tool Evidence route is working!"

@network_monitoring_tool_evidence_bp.route('/process_network_monitoring_tool_evidence', methods=['POST'])
@login_required
def process_network_monitoring_tool_evidence():
    print("="*50)
    print("NETWORK MONITOR TOOL EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR NETWORK MONITOR TOOL EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING NETWORK MONITOR TOOL EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "Network Monitor Tool Review.xlsx"

...
# Create blueprint
logical_access_controls_evidence_bp = Blueprint('logical_access_controls_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '29.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '29'
                if prefix == '29':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '29' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '29'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for Logical Access Controls Evidence"""
    mapping = {}
    ...
    # Create mappings for decimal numbers 1 to 3
    for decimal_num in range(1, 4):  # 1 to 3 inclusive
        # For decimal x, row number = x + 1
        # Decimal 1 -> 1 + 1 = 2, Decimal 2 -> 2 + 1 = 3, Decimal 3 -> 3 + 1 = 4
...
@logical_access_controls_evidence_bp.route('/test_logical_access_controls_evidence_route')
def test_logical_access_controls_evidence_route():
    return "Logical Access Controls Evidence route is working!"

@logical_access_controls_evidence_bp.route('/process_logical_access_controls_evidence', methods=['POST'])
@login_required
def process_logical_access_controls_evidence():
    print("="*50)
    print("LOGICAL ACCESS CONTROLS EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR LOGICAL ACCESS CONTROLS EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING LOGICAL ACCESS CONTROLS EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "Logical Access Controls Review.xlsx"

...
# Create blueprint
pam_evidence_bp = Blueprint('pam_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '42.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '42'
                if prefix == '42':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '42' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '42'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for PAM Evidence"""
    mapping = {}
...
@pam_evidence_bp.route('/test_pam_evidence_route')
def test_pam_evidence_route():
    return "PAM Evidence route is working!"

@pam_evidence_bp.route('/process_pam_evidence', methods=['POST'])
@login_required
def process_pam_evidence():
    print("="*50)
    print("PAM EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR PAM EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING PAM EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "PAM Review.xlsx"

...
# Create blueprint
data_input_control_evidence_bp = Blueprint('data_input_control_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '22.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '22'
                if prefix == '22':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '22' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '22'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for Data Input Controls Evidence"""
    mapping = {}
...
@data_input_control_evidence_bp.route('/test_data_input_control_evidence_route')
def test_data_input_control_evidence_route():
    return "Data Input Controls Evidence route is working!"

@data_input_control_evidence_bp.route('/process_data_input_control_evidence', methods=['POST'])
@login_required
def process_data_input_control_evidence():
    print("="*50)
    print("DATA INPUT CONTROLS EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR DATA INPUT CONTROLS EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING DATA INPUT CONTROLS EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "Data Input Control Review.xlsx"

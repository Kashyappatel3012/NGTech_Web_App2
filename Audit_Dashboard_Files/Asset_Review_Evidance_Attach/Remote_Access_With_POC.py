...
# Create blueprint
remote_access_evidence_bp = Blueprint('remote_access_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '33.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '33'
                if prefix == '33':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '33' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '33'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for Remote Access Evidence"""
    mapping = {}
    ...
    # Create mappings for decimal numbers 1 to 4
    for decimal_num in range(1, 5):  # 1 to 4 inclusive
        # For decimal x, row number = x + 1
        # Decimal 1 -> 1 + 1 = 2, Decimal 2 -> 2 + 1 = 3, Decimal 3 -> 3 + 1 = 4, Decimal 4 -> 4 + 1 = 5
...
@remote_access_evidence_bp.route('/test_remote_access_evidence_route')
def test_remote_access_evidence_route():
    return "Remote Access Evidence route is working!"

@remote_access_evidence_bp.route('/process_remote_access_evidence', methods=['POST'])
@login_required
def process_remote_access_evidence():
    print("="*50)
    print("REMOTE ACCESS EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR REMOTE ACCESS EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING REMOTE ACCESS EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "Remote Access Review.xlsx"

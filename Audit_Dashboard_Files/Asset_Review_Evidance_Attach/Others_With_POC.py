...
# Create blueprint
others_evidence_bp = Blueprint('others_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '46.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '46'
                if prefix == '46':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '46' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '46'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for Others Evidence"""
    mapping = {}
    ...
    # Decimal 7 -> J8, K8, I8
    # Decimal 8 -> J9, K9, I9
    # Decimal 9 -> J10, K10, I10
    # Decimal 10 -> J11, K11, I11
    # Decimal 11 -> J12, K12, I12
    # Decimal 12 -> J13, K13, I13
    # Decimal 13 -> J14, K14, I14
    # Decimal 14 -> J15, K15, I15
    # Decimal 15 -> J16, K16, I16
    # Decimal 16 -> J17, K17, I17
    # Decimal 17 -> J18, K18, I18
    
    # Create mappings for decimal numbers 1 to 17
    for decimal_num in range(1, 18):  # 1 to 17 inclusive
        # For decimal x, row number = x + 1
        # Decimal 1 -> 1 + 1 = 2, Decimal 2 -> 2 + 1 = 3, ..., Decimal 17 -> 17 + 1 = 18
...
@others_evidence_bp.route('/test_others_evidence_route')
def test_others_evidence_route():
    return "Others Evidence route is working!"

@others_evidence_bp.route('/process_others_evidence', methods=['POST'])
@login_required
def process_others_evidence():
    print("="*50)
    print("OTHERS EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR OTHERS EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING OTHERS EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            # Create the output filename (clean, no underscores)
            output_filename = "Others Review.xlsx"
            ...
            if os.path.exists(processed_file_path):
                ...
                # Flask's send_file with download_name will automatically set Content-Disposition header correctly
                response = send_file(
                    processed_file_path,
                    as_attachment=True,

...
# Create blueprint
database_controls_evidence_bp = Blueprint('database_controls_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '30.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '30'
                if prefix == '30':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '30' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '30'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for Database Controls Evidence"""
    mapping = {}
    ...
    # Create mappings for decimal numbers 1 to 14
    for decimal_num in range(1, 15):  # 1 to 14 inclusive
        # For decimal x, row number = x + 1
        # Decimal 1 -> 1 + 1 = 2, Decimal 2 -> 2 + 1 = 3, ..., Decimal 14 -> 14 + 1 = 15
...
@database_controls_evidence_bp.route('/test_database_controls_evidence_route')
def test_database_controls_evidence_route():
    return "Database Controls Evidence route is working!"

@database_controls_evidence_bp.route('/process_database_controls_evidence', methods=['POST'])
@login_required
def process_database_controls_evidence():
    print("="*50)
    print("DATABASE CONTROLS EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR DATABASE CONTROLS EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING DATABASE CONTROLS EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "Database Controls Review.xlsx"

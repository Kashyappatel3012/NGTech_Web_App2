...
# Create blueprint
power_supply_evidence_bp = Blueprint('power_supply_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '34.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '34'
                if prefix == '34':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '34' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '34'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for Power Supply (UPS) Evidence"""
    mapping = {}
    ...
    for decimal_num in range(1, 6):  # 1 to 5 inclusive
        ...
        # Decimal 1 -> 1 + 1 = 2, Decimal 2 -> 2 + 1 = 3, Decimal 3 -> 3 + 1 = 4, Decimal 4 -> 4 + 1 = 5, Decimal 5 -> 5 + 1 = 6
...
@power_supply_evidence_bp.route('/test_power_supply_evidence_route')
def test_power_supply_evidence_route():
    return "Power Supply Evidence route is working!"

@power_supply_evidence_bp.route('/process_power_supply_evidence', methods=['POST'])
@login_required
def process_power_supply_evidence():
    print("="*50)
    print("POWER SUPPLY EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR POWER SUPPLY EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING POWER SUPPLY EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "Power Supply UPS Review.xlsx"

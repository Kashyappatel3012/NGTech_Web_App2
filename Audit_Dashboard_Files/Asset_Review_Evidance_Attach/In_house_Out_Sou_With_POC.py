...
# Create blueprint
inhouse_outsourced_evidence_bp = Blueprint('inhouse_outsourced_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '25.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '25'
                if prefix == '25':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '25' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '25'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for In-house and Out-sourced Evidence"""
    mapping = {}
    ...
    # Create mappings for decimal numbers 1 to 9
    for decimal_num in range(1, 10):  # 1 to 9 inclusive
        # For decimal x, row number = x + 1
        # Decimal 1 -> 1 + 1 = 2, Decimal 2 -> 2 + 1 = 3, ..., Decimal 9 -> 9 + 1 = 10
...
@inhouse_outsourced_evidence_bp.route('/test_inhouse_outsourced_evidence_route')
def test_inhouse_outsourced_evidence_route():
    return "In-house and Out-sourced Evidence route is working!"

@inhouse_outsourced_evidence_bp.route('/process_inhouse_outsourced_evidence', methods=['POST'])
@login_required
def process_inhouse_outsourced_evidence():
    print("="*50)
    print("IN-HOUSE AND OUT-SOURCED EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR IN-HOUSE AND OUT-SOURCED EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING IN-HOUSE AND OUT-SOURCED EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "In-house and Out-sourced Review.xlsx"

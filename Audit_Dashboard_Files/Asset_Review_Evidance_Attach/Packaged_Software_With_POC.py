...
# Create blueprint
packaged_software_evidence_bp = Blueprint('packaged_software_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '27.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '27'
                if prefix == '27':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '27' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '27'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for Packaged Software Evidence"""
    mapping = {}
...
@packaged_software_evidence_bp.route('/test_packaged_software_evidence_route')
def test_packaged_software_evidence_route():
    return "Packaged Software Evidence route is working!"

@packaged_software_evidence_bp.route('/process_packaged_software_evidence', methods=['POST'])
@login_required
def process_packaged_software_evidence():
    print("="*50)
    print("PACKAGED SOFTWARE EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR PACKAGED SOFTWARE EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING PACKAGED SOFTWARE EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "Packaged Software Review.xlsx"

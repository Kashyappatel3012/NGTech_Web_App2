...
# Create blueprint
antivirus_evidence_bp = Blueprint('antivirus_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '9.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '9'
                if prefix == '9':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '9' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '9'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for Antivirus Evidence"""
    mapping = {}
...
@antivirus_evidence_bp.route('/test_antivirus_evidence_route')
def test_antivirus_evidence_route():
    return "Antivirus Evidence route is working!"

@antivirus_evidence_bp.route('/process_antivirus_evidence', methods=['POST'])
@login_required
def process_antivirus_evidence():
    print("="*50)
    print("ANTIVIRUS EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR ANTIVIRUS EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING ANTIVIRUS EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "Antivirus Review.xlsx"

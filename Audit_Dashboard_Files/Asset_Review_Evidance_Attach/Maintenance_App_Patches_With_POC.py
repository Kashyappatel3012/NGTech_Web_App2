...
# Create blueprint
maintenance_patches_evidence_bp = Blueprint('maintenance_patches_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '36.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '36'
                if prefix == '36':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '36' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '36'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for Maintenance & App Patches Evidence"""
    mapping = {}
    ...
    # Create mappings for decimal numbers 1 to 8
    for decimal_num in range(1, 9):  # 1 to 8 inclusive
        # For decimal x, row number = x + 1
        # Decimal 1 -> 1 + 1 = 2, Decimal 2 -> 2 + 1 = 3, ..., Decimal 8 -> 8 + 1 = 9
...
@maintenance_patches_evidence_bp.route('/test_maintenance_patches_evidence_route')
def test_maintenance_patches_evidence_route():
    return "Maintenance & App Patches Evidence route is working!"

@maintenance_patches_evidence_bp.route('/process_maintenance_patches_evidence', methods=['POST'])
@login_required
def process_maintenance_patches_evidence():
    print("="*50)
    print("MAINTENANCE & APP PATCHES EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR MAINTENANCE & APP PATCHES EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING MAINTENANCE & APP PATCHES EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "Maintenance and App Patches Review.xlsx"

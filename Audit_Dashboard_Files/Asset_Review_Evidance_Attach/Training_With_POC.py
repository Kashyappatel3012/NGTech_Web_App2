...
# Create blueprint
training_evidence_bp = Blueprint('training_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '32.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '32'
                if prefix == '32':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '32' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '32'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for Training Evidence"""
    mapping = {}
...
@training_evidence_bp.route('/test_training_evidence_route')
def test_training_evidence_route():
    return "Training Evidence route is working!"

@training_evidence_bp.route('/process_training_evidence', methods=['POST'])
@login_required
def process_training_evidence():
    print("="*50)
    print("TRAINING EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR TRAINING EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING TRAINING EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "Training Review.xlsx"

...
# Create blueprint
nas_evidence_bp = Blueprint('nas_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '40.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '40'
                if prefix == '40':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '40' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '40'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for NAS Evidence"""
    mapping = {}
    ...
    # Create mappings for decimal numbers 1 to 11
    for decimal_num in range(1, 12):  # 1 to 11 inclusive
        # For decimal x, row number = x + 1
        # Decimal 1 -> 1 + 1 = 2, Decimal 2 -> 2 + 1 = 3, ..., Decimal 11 -> 11 + 1 = 12
...
@nas_evidence_bp.route('/test_nas_evidence_route')
def test_nas_evidence_route():
    return "NAS Evidence route is working!"

@nas_evidence_bp.route('/process_nas_evidence', methods=['POST'])
@login_required
def process_nas_evidence():
    print("="*50)
    print("NAS EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR NAS EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING NAS EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "NAS Review.xlsx"

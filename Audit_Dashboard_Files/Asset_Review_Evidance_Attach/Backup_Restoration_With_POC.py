...
# Create blueprint
backup_restoration_evidence_bp = Blueprint('backup_restoration_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '35.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '35'
                if prefix == '35':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '35' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '35'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for Backup and Restoration Evidence"""
    mapping = {}
...
@backup_restoration_evidence_bp.route('/test_backup_restoration_evidence_route')
def test_backup_restoration_evidence_route():
    return "Backup and Restoration Evidence route is working!"

@backup_restoration_evidence_bp.route('/process_backup_restoration_evidence', methods=['POST'])
@login_required
def process_backup_restoration_evidence():
    print("="*50)
    print("BACKUP AND RESTORATION EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR BACKUP AND RESTORATION EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING BACKUP AND RESTORATION EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "Backup and Restoration Review.xlsx"

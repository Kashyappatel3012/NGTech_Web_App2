...
# Create blueprint
access_control_application_evidence_bp = Blueprint('access_control_application_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '16.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '16'
                if prefix == '16':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '16' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '16'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for Access Control Application Evidence"""
    mapping = {}
    ...
    # Create mappings for decimal numbers 1 to 12
    for decimal_num in range(1, 13):  # 1 to 12 inclusive
        # For decimal x, row number = x + 1
        # Decimal 1 -> 1 + 1 = 2, Decimal 2 -> 2 + 1 = 3, Decimal 12 -> 12 + 1 = 13
...
@access_control_application_evidence_bp.route('/test_access_control_application_evidence_route')
def test_access_control_application_evidence_route():
    return "Access Control Application Evidence route is working!"

@access_control_application_evidence_bp.route('/process_access_control_application_evidence', methods=['POST'])
@login_required
def process_access_control_application_evidence():
    print("="*50)
    print("ACCESS CONTROL APPLICATION EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR ACCESS CONTROL APPLICATION EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING ACCESS CONTROL APPLICATION EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "Access Control Application Review.xlsx"

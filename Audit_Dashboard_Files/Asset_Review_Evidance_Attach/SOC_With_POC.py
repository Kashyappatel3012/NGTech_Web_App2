...
# Create blueprint
soc_evidence_bp = Blueprint('soc_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '43.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '43'
                if prefix == '43':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '43' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '43'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for SOC Evidence"""
    mapping = {}
    ...
    # Decimal 18 -> J19, K19, I19
    # Decimal 19 -> J20, K20, I20
    # Decimal 20 -> J21, K21, I21
    # Decimal 21 -> J22, K22, I22
    # Decimal 22 -> J23, K23, I23
    
    # Create mappings for decimal numbers 1 to 22
    for decimal_num in range(1, 23):  # 1 to 22 inclusive
        # For decimal x, row number = x + 1
        # Decimal 1 -> 1 + 1 = 2, Decimal 2 -> 2 + 1 = 3, ..., Decimal 22 -> 22 + 1 = 23
...
@soc_evidence_bp.route('/test_soc_evidence_route')
def test_soc_evidence_route():
    return "SOC Evidence route is working!"

@soc_evidence_bp.route('/process_soc_evidence', methods=['POST'])
@login_required
def process_soc_evidence():
    print("="*50)
    print("SOC EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR SOC EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING SOC EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "SOC Review.xlsx"

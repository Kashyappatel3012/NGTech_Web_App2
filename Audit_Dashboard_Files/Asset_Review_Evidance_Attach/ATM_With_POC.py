...
# Create blueprint
atm_evidence_bp = Blueprint('atm_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '10.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '10'
                if prefix == '10':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '10' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '10'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for ATM Evidence"""
    mapping = {}
    ...
    # Decimal 3 -> J4, K4, I4
    # .........
    # Decimal 24 -> J25, K25, I25
    
    # Create mappings for decimal numbers 1 to 24
    for decimal_num in range(1, 25):  # 1 to 24 inclusive
...
@atm_evidence_bp.route('/test_atm_evidence_route')
def test_atm_evidence_route():
    return "ATM Evidence route is working!"

@atm_evidence_bp.route('/process_atm_evidence', methods=['POST'])
@login_required
def process_atm_evidence():
    print("="*50)
    print("ATM EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR ATM EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING ATM EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "ATM Review.xlsx"

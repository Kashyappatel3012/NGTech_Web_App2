...
# Create blueprint
disaster_recovery_evidence_bp = Blueprint('disaster_recovery_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '3.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '3'
                if prefix == '3':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '3' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '3'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for Disaster Recovery Evidence"""
    mapping = {}
    ...
    # Decimal 3 -> J4, K4, I4
    # .........
    # Decimal 47 -> J48, K48, I48
    
    # Create mappings for decimal numbers 1 to 47
    for decimal_num in range(1, 48):  # 1 to 47 inclusive
...
@disaster_recovery_evidence_bp.route('/test_disaster_recovery_evidence_route')
def test_disaster_recovery_evidence_route():
    return "Disaster Recovery Evidence route is working!"

@disaster_recovery_evidence_bp.route('/process_disaster_recovery_evidence', methods=['POST'])
@login_required
def process_disaster_recovery_evidence():
    print("="*50)
    print("DISASTER RECOVERY EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR DISASTER RECOVERY EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING DISASTER RECOVERY EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "Disaster Recovery Review.xlsx"

...
# Create blueprint
ho_win_server_evidence_bp = Blueprint('ho_win_server_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '12.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '12'
                if prefix == '12':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '12' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '12'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for HO Win_Server Logical Review Evidence"""
    mapping = {}
    ...
    # Decimal 3 -> J4, K4, I4
    # .........
    # Decimal 42 -> J43, K43, I43
    
    # Create mappings for decimal numbers 1 to 42
    for decimal_num in range(1, 43):  # 1 to 42 inclusive
...
@ho_win_server_evidence_bp.route('/test_ho_win_server_evidence_route')
def test_ho_win_server_evidence_route():
    return "HO Win_Server Evidence route is working!"

@ho_win_server_evidence_bp.route('/process_ho_win_server_evidence', methods=['POST'])
@login_required
def process_ho_win_server_evidence():
    print("="*50)
    print("HO WIN_SERVER EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR HO WIN_SERVER EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING HO WIN_SERVER EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "HO Win Server Logical Review.xlsx"

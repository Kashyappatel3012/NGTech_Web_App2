...
# Create blueprint
access_control_os_evidence_bp = Blueprint('access_control_os_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '15.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '15'
                if prefix == '15':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '15' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '15'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for Access Control – OS Level Evidence"""
    mapping = {}
...
@access_control_os_evidence_bp.route('/test_access_control_os_evidence_route')
def test_access_control_os_evidence_route():
    return "Access Control – OS Level Evidence route is working!"

@access_control_os_evidence_bp.route('/process_access_control_os_evidence', methods=['POST'])
@login_required
def process_access_control_os_evidence():
    print("="*50)
    print("ACCESS CONTROL – OS LEVEL EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR ACCESS CONTROL – OS LEVEL EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING ACCESS CONTROL – OS LEVEL EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "Access Control OS Level Review.xlsx"

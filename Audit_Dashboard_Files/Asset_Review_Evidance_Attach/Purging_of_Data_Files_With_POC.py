...
# Create blueprint
purging_data_files_evidence_bp = Blueprint('purging_data_files_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '23.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '23'
                if prefix == '23':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '23' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '23'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for Purging of Data Files Evidence"""
    mapping = {}
...
@purging_data_files_evidence_bp.route('/test_purging_data_files_evidence_route')
def test_purging_data_files_evidence_route():
    return "Purging of Data Files Evidence route is working!"

@purging_data_files_evidence_bp.route('/process_purging_data_files_evidence', methods=['POST'])
@login_required
def process_purging_data_files_evidence():
    print("="*50)
    print("PURGING OF DATA FILES EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR PURGING OF DATA FILES EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING PURGING OF DATA FILES EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "Purging of Data Files Review.xlsx"

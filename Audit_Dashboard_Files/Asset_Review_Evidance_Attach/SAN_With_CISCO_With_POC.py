...
# Create blueprint
san_switch_cisco_evidence_bp = Blueprint('san_switch_cisco_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '38.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '38'
                if prefix == '38':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '38' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '38'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for SAN Switch CISCO Evidence"""
    mapping = {}
...
@san_switch_cisco_evidence_bp.route('/test_san_switch_cisco_evidence_route')
def test_san_switch_cisco_evidence_route():
    return "SAN Switch CISCO Evidence route is working!"

@san_switch_cisco_evidence_bp.route('/process_san_switch_cisco_evidence', methods=['POST'])
@login_required
def process_san_switch_cisco_evidence():
    print("="*50)
    print("SAN SWITCH CISCO EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR SAN SWITCH CISCO EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING SAN SWITCH CISCO EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "SAN Switch CISCO Review.xlsx"

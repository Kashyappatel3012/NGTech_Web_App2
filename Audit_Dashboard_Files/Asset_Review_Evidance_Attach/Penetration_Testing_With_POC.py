...
# Create blueprint
penetration_testing_evidence_bp = Blueprint('penetration_testing_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '31.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '31'
                if prefix == '31':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '31' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '31'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for Penetration Testing Evidence"""
    mapping = {}
...
@penetration_testing_evidence_bp.route('/test_penetration_testing_evidence_route')
def test_penetration_testing_evidence_route():
    return "Penetration Testing Evidence route is working!"

@penetration_testing_evidence_bp.route('/process_penetration_testing_evidence', methods=['POST'])
@login_required
def process_penetration_testing_evidence():
    print("="*50)
    print("PENETRATION TESTING EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR PENETRATION TESTING EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING PENETRATION TESTING EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "Penetration Testing Review.xlsx"

...
# Create blueprint
linux_server_evidence_bp = Blueprint('linux_server_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '13.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '13'
                if prefix == '13':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '13' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '13'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for Linux_Server_Logical Review Evidence"""
    mapping = {}
    ...
    # Decimal 3 -> J4, K4, I4
    # .........
    # Decimal 54 -> J55, K55, I55
    
    # Create mappings for decimal numbers 1 to 54
    for decimal_num in range(1, 55):  # 1 to 54 inclusive
...
@linux_server_evidence_bp.route('/test_linux_server_evidence_route')
def test_linux_server_evidence_route():
    return "Linux_Server Evidence route is working!"

@linux_server_evidence_bp.route('/process_linux_server_evidence', methods=['POST'])
@login_required
def process_linux_server_evidence():
    print("="*50)
    print("LINUX_SERVER EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR LINUX_SERVER EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING LINUX_SERVER EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "Linux Server Logical Review.xlsx"

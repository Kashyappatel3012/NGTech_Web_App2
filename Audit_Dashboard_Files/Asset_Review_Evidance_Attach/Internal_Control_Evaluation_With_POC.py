...
# Create blueprint
internal_control_evidence_bp = Blueprint('internal_control_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '19.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '19'
                if prefix == '19':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '19' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '19'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for Internal Control Evaluation Evidence"""
    mapping = {}
...
@internal_control_evidence_bp.route('/test_internal_control_evidence_route')
def test_internal_control_evidence_route():
    return "Internal Control Evaluation Evidence route is working!"

@internal_control_evidence_bp.route('/process_internal_control_evidence', methods=['POST'])
@login_required
def process_internal_control_evidence():
    print("="*50)
    print("INTERNAL CONTROL EVALUATION EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR INTERNAL CONTROL EVALUATION EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING INTERNAL CONTROL EVALUATION EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "Internal Control Evaluation Review.xlsx"

...
# Create blueprint
data_centre_evidence_bp = Blueprint('data_centre_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '2.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '2'
                if prefix == '2':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '2' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '2'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for Data Centre Evidence"""
    mapping = {}
    ...
    # Decimal 3 -> J4, K4, I4
    # .........
    # Decimal 45 -> J46, K46, I46
    
    # Create mappings for decimal numbers 1 to 45
    for decimal_num in range(1, 46):  # 1 to 45 inclusive
...
@data_centre_evidence_bp.route('/test_data_centre_evidence_route')
def test_data_centre_evidence_route():
    return "Data Centre Evidence route is working!"

@data_centre_evidence_bp.route('/process_data_centre_evidence', methods=['POST'])
@login_required
def process_data_centre_evidence():
    print("="*50)
    print("DATA CENTRE EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR DATA CENTRE EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING DATA CENTRE EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "Data Centre Review.xlsx"

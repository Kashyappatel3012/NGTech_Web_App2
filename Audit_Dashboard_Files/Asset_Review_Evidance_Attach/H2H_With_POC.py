...
# Create blueprint
h2h_evidence_bp = Blueprint('h2h_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '8.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '8'
                if prefix == '8':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '8' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '8'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for H2H Audit Evidence"""
    mapping = {}
    ...
    # Decimal 3 -> J4, K4, I4
    # .........
    # Decimal 23 -> J24, K24, I24
    
    # Create mappings for decimal numbers 1 to 23
    for decimal_num in range(1, 24):  # 1 to 23 inclusive
...
@h2h_evidence_bp.route('/test_h2h_evidence_route')
def test_h2h_evidence_route():
    return "H2H Audit Evidence route is working!"

@h2h_evidence_bp.route('/process_h2h_evidence', methods=['POST'])
@login_required
def process_h2h_evidence():
    print("="*50)
    print("H2H AUDIT EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR H2H AUDIT EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING H2H AUDIT EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "H2H Audit Review.xlsx"

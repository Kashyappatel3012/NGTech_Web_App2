...
# Create blueprint
fire_protection_evidence_bp = Blueprint('fire_protection_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '20.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '20'
                if prefix == '20':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '20' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '20'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for Fire Protection Evidence"""
    mapping = {}
...
@fire_protection_evidence_bp.route('/test_fire_protection_evidence_route')
def test_fire_protection_evidence_route():
    return "Fire Protection Evidence route is working!"

@fire_protection_evidence_bp.route('/process_fire_protection_evidence', methods=['POST'])
@login_required
def process_fire_protection_evidence():
    print("="*50)
    print("FIRE PROTECTION EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR FIRE PROTECTION EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING FIRE PROTECTION EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "Fire Protection Review.xlsx"

...
# Create blueprint
domain_controller_evidence_bp = Blueprint('domain_controller_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '7.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '7'
                if prefix == '7':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '7' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '7'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for Domain Controller Evidence"""
    mapping = {}
    ...
    # Decimal 3 -> J4, K4, I4
    # .........
    # Decimal 26 -> J27, K27, I27
    
    # Create mappings for decimal numbers 1 to 26
    for decimal_num in range(1, 27):  # 1 to 26 inclusive
...
@domain_controller_evidence_bp.route('/test_domain_controller_evidence_route')
def test_domain_controller_evidence_route():
    return "Domain Controller Evidence route is working!"

@domain_controller_evidence_bp.route('/process_domain_controller_evidence', methods=['POST'])
@login_required
def process_domain_controller_evidence():
    print("="*50)
    print("DOMAIN CONTROLLER EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR DOMAIN CONTROLLER EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING DOMAIN CONTROLLER EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "Domain Controller AD Review.xlsx"

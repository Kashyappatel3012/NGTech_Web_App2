...
# Create blueprint
router_evidence_bp = Blueprint('router_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '6.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '6'
                if prefix == '6':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '6' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '6'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for Router Evidence"""
    mapping = {}
...
@router_evidence_bp.route('/test_router_evidence_route')
def test_router_evidence_route():
    return "Router Evidence route is working!"

@router_evidence_bp.route('/process_router_evidence', methods=['POST'])
@login_required
def process_router_evidence():
    print("="*50)
    print("ROUTER EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR ROUTER EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING ROUTER EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "Router Review.xlsx"

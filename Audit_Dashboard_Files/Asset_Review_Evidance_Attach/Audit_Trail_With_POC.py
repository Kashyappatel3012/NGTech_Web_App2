...
# Create blueprint
audit_trail_evidence_bp = Blueprint('audit_trail_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '26.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '26'
                if prefix == '26':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '26' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '26'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for Audit Trail Evidence"""
    mapping = {}
...
@audit_trail_evidence_bp.route('/test_audit_trail_evidence_route')
def test_audit_trail_evidence_route():
    return "Audit Trail Evidence route is working!"

@audit_trail_evidence_bp.route('/process_audit_trail_evidence', methods=['POST'])
@login_required
def process_audit_trail_evidence():
    print("="*50)
    print("AUDIT TRAIL EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR AUDIT TRAIL EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING AUDIT TRAIL EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "Audit Trail Review.xlsx"

...
# Create blueprint
asset_management_evidence_bp = Blueprint('asset_management_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '45.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '45'
                if prefix == '45':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '45' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '45'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for Asset Management Evidence"""
    mapping = {}
...
@asset_management_evidence_bp.route('/test_asset_management_evidence_route')
def test_asset_management_evidence_route():
    return "Asset Management Evidence route is working!"

@asset_management_evidence_bp.route('/process_asset_management_evidence', methods=['POST'])
@login_required
def process_asset_management_evidence():
    print("="*50)
    print("ASSET MANAGEMENT EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR ASSET MANAGEMENT EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING ASSET MANAGEMENT EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "Asset Management Review.xlsx"

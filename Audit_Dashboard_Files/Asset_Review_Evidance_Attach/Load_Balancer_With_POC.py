...
# Create blueprint
load_balancer_array_evidence_bp = Blueprint('load_balancer_array_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '41.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '41'
                if prefix == '41':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '41' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '41'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for Load Balancer Array Evidence"""
    mapping = {}
...
@load_balancer_array_evidence_bp.route('/test_load_balancer_array_evidence_route')
def test_load_balancer_array_evidence_route():
    return "Load Balancer Array Evidence route is working!"

@load_balancer_array_evidence_bp.route('/process_load_balancer_array_evidence', methods=['POST'])
@login_required
def process_load_balancer_array_evidence():
    print("="*50)
    print("LOAD BALANCER ARRAY EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR LOAD BALANCER ARRAY EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING LOAD BALANCER ARRAY EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "Load Balancer Array Review.xlsx"

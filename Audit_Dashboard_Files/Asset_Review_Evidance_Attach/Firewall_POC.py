...
# Create blueprint
firewall_evidence_bp = Blueprint('firewall_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '4.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '4'
                if prefix == '4':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '4' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '4'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for Firewall Evidence"""
    mapping = {}
    ...
    # Decimal 3 -> J4, K4, I4
    # .........
    # Decimal 34 -> J35, K35, I35
    
    # Create mappings for decimal numbers 1 to 34
    for decimal_num in range(1, 35):  # 1 to 34 inclusive
...
@firewall_evidence_bp.route('/test_firewall_evidence_route')
def test_firewall_evidence_route():
    return "Firewall Evidence route is working!"

@firewall_evidence_bp.route('/process_firewall_evidence', methods=['POST'])
@login_required
def process_firewall_evidence():
    print("="*50)
    print("FIREWALL EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR FIREWALL EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING FIREWALL EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "Firewall Review.xlsx"

...
# Create blueprint
user_account_maintenance_evidence_bp = Blueprint('user_account_maintenance_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '28.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '28'
                if prefix == '28':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '28' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '28'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for User Account Maintenance Evidence"""
    mapping = {}
    ...
    # Create mappings for decimal numbers 1 to 5
    for decimal_num in range(1, 6):  # 1 to 5 inclusive
        # For decimal x, row number = x + 1
        # Decimal 1 -> 1 + 1 = 2, Decimal 2 -> 2 + 1 = 3, ..., Decimal 5 -> 5 + 1 = 6
...
@user_account_maintenance_evidence_bp.route('/test_user_account_maintenance_evidence_route')
def test_user_account_maintenance_evidence_route():
    return "User Account Maintenance Evidence route is working!"

@user_account_maintenance_evidence_bp.route('/process_user_account_maintenance_evidence', methods=['POST'])
@login_required
def process_user_account_maintenance_evidence():
    print("="*50)
    print("USER ACCOUNT MAINTENANCE EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR USER ACCOUNT MAINTENANCE EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING USER ACCOUNT MAINTENANCE EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "User Account Maintenance Review.xlsx"

...
# Create blueprint
business_continuity_planning_evidence_bp = Blueprint('business_continuity_planning_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '24.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '24'
                if prefix == '24':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '24' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '24'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for Business Continuity Planning Evidence"""
    mapping = {}
    ...
    # Create mappings for decimal numbers 1 to 7
    for decimal_num in range(1, 8):  # 1 to 7 inclusive
        # For decimal x, row number = x + 1
        # Decimal 1 -> 1 + 1 = 2, Decimal 2 -> 2 + 1 = 3, ..., Decimal 7 -> 7 + 1 = 8
...
@business_continuity_planning_evidence_bp.route('/test_business_continuity_planning_evidence_route')
def test_business_continuity_planning_evidence_route():
    return "Business Continuity Planning Evidence route is working!"

@business_continuity_planning_evidence_bp.route('/process_business_continuity_planning_evidence', methods=['POST'])
@login_required
def process_business_continuity_planning_evidence():
    print("="*50)
    print("BUSINESS CONTINUITY PLANNING EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR BUSINESS CONTINUITY PLANNING EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING BUSINESS CONTINUITY PLANNING EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "Business Continuity Planning Review.xlsx"

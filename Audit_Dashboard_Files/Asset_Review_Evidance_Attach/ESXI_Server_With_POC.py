...
# Create blueprint
esxi_server_evidence_bp = Blueprint('esxi_server_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '14.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '14'
                if prefix == '14':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '14' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '14'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for ESXi Server_Logical Review Evidence"""
    mapping = {}
    ...
    # Create mappings for decimal numbers 1 to 15
    for decimal_num in range(1, 16):  # 1 to 15 inclusive
        # For decimal x, row number = x + 1
        # Decimal 1 -> 1 + 1 = 2, Decimal 2 -> 2 + 1 = 3, Decimal 15 -> 15 + 1 = 16
...
@esxi_server_evidence_bp.route('/test_esxi_server_evidence_route')
def test_esxi_server_evidence_route():
    return "ESXi_Server Evidence route is working!"

@esxi_server_evidence_bp.route('/process_esxi_server_evidence', methods=['POST'])
@login_required
def process_esxi_server_evidence():
    print("="*50)
    print("ESXI_SERVER EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR ESXI_SERVER EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING ESXI_SERVER EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "ESXi Server Logical Review.xlsx"

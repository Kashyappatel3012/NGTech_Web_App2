...
# Create blueprint
network_review_evidence_bp = Blueprint('network_review_evidence_bp', __name__)
...
def extract_base_number(filename):
    """Extract only the decimal part from filename for image placement logic, only if it starts with '1.'"""
    try:
        ...
        if '.' in base_part:
            ...
            if len(parts) >= 2:
                ...
                # Only process if the prefix is '1'
                if prefix == '1':
                    result = int(decimal_part)
                    print(f"Decimal part extracted: {decimal_part} -> {result} (prefix '1' confirmed)")
                    return result
                else:
                    print(f"Ignoring filename {filename} - prefix '{prefix}' is not '1'")
                    return None
            else:
...
def create_image_cell_mapping():
    """Create mapping from decimal numbers to Excel cells for Network Review Evidence"""
    mapping = {}
    ...
    # Decimal 3 -> J4, K4, I4
    # .........
    # Decimal 31 -> J32, K32, I32
    # Decimal 32 -> J33, K33, I33
    # Decimal 33 -> J34, K34, I34
    
    # Create mappings for decimal numbers 1 to 33
    for decimal_num in range(1, 34):  # 1 to 33 inclusive
        # For decimal x, row number = x + 1
        # Decimal 1 -> 1 + 1 = 2, Decimal 2 -> 2 + 1 = 3, Decimal 21 -> 21 + 1 = 22
...
@network_review_evidence_bp.route('/test_network_review_evidence_route')
def test_network_review_evidence_route():
    return "Network Review Evidence route is working!"

@network_review_evidence_bp.route('/process_network_review_evidence', methods=['POST'])
@login_required
def process_network_review_evidence():
    print("="*50)
    print("NETWORK REVIEW EVIDENCE FORM SUBMISSION RECEIVED!")
    print("="*50)
    ...
    try:
        ...
        try:
            ...
            print("\n" + "="*60)
            print("PROCESSING ZIP FILE FOR NETWORK REVIEW EVIDENCE IMAGES")
            print("="*60)
            ...
            if extracted_images:
                print("\n" + "="*60)
                print("INSERTING NETWORK REVIEW EVIDENCE IMAGES INTO EXCEL")
                print("="*60)
            ...
            output_filename = "Network Review.xlsx"

...
# Create blueprint for Follow Up Audit MetaData
follow_up_audit_metadata_bp = Blueprint('follow_up_audit_metadata', __name__)

def create_follow_up_audit_metadata_excel(form_data):
    ...
    try:
        ...
        filename = f'Follow_Up_Audit_MetaData.xlsx'
...
@follow_up_audit_metadata_bp.route('/process_follow_up_audit_metadata', methods=['POST'])
def process_follow_up_audit_metadata():
    """Process Follow Up Audit MetaData form submission"""
    try:
        print("\n" + "="*80)
        ...
        # Create Excel file
        filepath, filename = create_follow_up_audit_metadata_excel(form_data)
...
# Cleanup endpoint
@follow_up_audit_metadata_bp.route('/cleanup_follow_up_audit_metadata', methods=['POST'])
def cleanup_follow_up_audit_metadata():
    """Clean up old Follow Up Audit MetaData Excel files"""
    try:
        uploads_dir = os.path.join('static', 'uploads')
        if os.path.exists(uploads_dir):
            for filename in os.listdir(uploads_dir):
                if filename.startswith('Follow_Up_Audit_MetaData_') and filename.endswith('.xlsx'):

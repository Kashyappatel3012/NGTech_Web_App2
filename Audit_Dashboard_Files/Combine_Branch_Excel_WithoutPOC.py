...
# Create blueprint
combine_branch_excel_without_poc_bp = Blueprint('combine_branch_excel_without_poc_bp', __name__)
...
def extract_leading_number(filename):
    """
    Extract the leading number from a filename for sorting purposes.
    Returns a tuple (number, original_filename) where number is an integer.
    If no number is found at the start, returns (999999, original_filename) to put it at the end.
    
    Examples:
        "1 Shilaj Branch.xlsx" -> (1, "1 Shilaj Branch.xlsx")
        "2 Udapi Branch.xlsx" -> (2, "2 Udapi Branch.xlsx")
        "10 Some Branch.xlsx" -> (10, "10 Some Branch.xlsx")
        "Branch Name.xlsx" -> (999999, "Branch Name.xlsx")
    """
    # Remove file extension
    name_without_ext = os.path.splitext(filename)[0]
    
    # Try to match a number at the beginning of the filename
    match = re.match(r'^(\d+)', name_without_ext.strip())
...
def sort_excel_files_by_number(excel_files):
    """
    Sort Excel files by the leading number in their filename.
    Files with numbers come first (sorted numerically), files without numbers come last.
    """
    sorted_files = sorted(excel_files, key=lambda x: extract_leading_number(x['filename']))
    return sorted_files
...
def combine_excel_files_to_bytes(excel_files):
    ...
    try:
                    ...
                    for row in source_sheet.iter_rows():
                        for cell in row:
                            ...
                            try:
                                ...
                                new_cell.value = sanitize_excel_value(cell.value)
                            except Exception as value_error:
                                print(f"Warning: Could not copy value from cell {cell.coordinate}: {value_error}")
                                from excel_security_utils import sanitize_excel_value
                                new_cell.value = sanitize_excel_value(str(cell.value)) if cell.value is not None else None
        ...
        try:
            ...
            combined_wb.properties.subject = "Branch Excel Without Evidence"
...
@combine_branch_excel_without_poc_bp.route('/test_combine_route')
def test_combine_route():
    return "Combine Branch Excel Without POC route is working!"

@combine_branch_excel_without_poc_bp.route('/combine_branch_excel_without_poc', methods=['POST'])
@login_required
def combine_branch_excel_without_poc():
    print("="*50)
    print("COMBINE BRANCH EXCEL WITHOUT POC FORM SUBMISSION RECEIVED!")
    print("="*50)
        ...
        try:
            ...
            if extracted_files:
                ...
                if excel_files:
                    ...
                    # Sort Excel files by leading number in filename
                    print("\n" + "="*60)
                    print("SORTING EXCEL FILES BY LEADING NUMBER")
                    print("="*60)
                    print("Before sorting:")
                    for i, file_info in enumerate(excel_files, 1):
                        print(f"  {i}. {file_info['filename']}")
                    
                    sorted_excel_files = sort_excel_files_by_number(excel_files)
                    
                    print("\nAfter sorting:")
                    for i, file_info in enumerate(sorted_excel_files, 1):
                        number, _ = extract_leading_number(file_info['filename'])
                        print(f"  {i}. {file_info['filename']} (number: {number if number != 999999 else 'N/A'})")
                    print("="*60)
                    
                    # Create output filename
                    output_filename = "Branch_Excel_Without_Evidence.xlsx"
                    
                    # Combine Excel files to memory (using sorted list)
                    excel_data, sheets_added = combine_excel_files_to_bytes(sorted_excel_files)

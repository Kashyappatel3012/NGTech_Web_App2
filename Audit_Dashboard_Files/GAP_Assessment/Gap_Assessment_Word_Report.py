...
# Create blueprint for Gap Assessment Word Report
gap_assessment_report_bp = Blueprint('gap_assessment_report', __name__)

def get_ordinal_superscript(day):
...
def format_date_with_superscript(date_str):
    ...
    try:
        ...
        day_with_ordinal = get_ordinal_superscript(day)
...
def get_financial_year(date_str):
    ...
    try:
        ...
        start_formatted = f"{get_ordinal_superscript(1)} April {fy_start_year}"
        end_formatted = f"{get_ordinal_superscript(31)} March {fy_end_year}"
        
        return f"{start_formatted} to {end_formatted}"
    except:
        return ""

def check_loc_cells(ws):
...
def get_areas_covered_text(level):
...
def _replace_text_with_left_indent(paragraph, old_text, new_text):
    """Replace text and add left indent for areas covered list"""
    if not _replace_text_in_runs(paragraph, old_text, new_text):
        return False
            ...
            for old_text, new_text in replacements.items():
                if old_text in paragraph.text:
                    if old_text == "Aarreas_Covvered":
                        _replace_text_with_left_indent(paragraph, old_text, new_text)
                    else:
...
def insert_excel_table_in_document(doc, placeholder, ws, start_col, end_col, start_row, end_row):
    ...
    try:
        ...
        try:
            ...
            header_width_map = {
                ...
                "Max Marks": 15,
                "Yes/No": 18,
                "Marks given by the Auditor": 15,
                "Marks given by Bank": 15,
                "Yes/No given by the Bank": 18,
...
def sort_images(image_files):
    ...
    parsed_images = [parse_image_name(img) for img in image_files]
...
def insert_images_as_annexures(doc, placeholder, sorted_images, image_paths):
...
@gap_assessment_report_bp.route('/process_gap_assessment_report', methods=['POST'])
def process_gap_assessment_report():
    """Process Gap Assessment Report generation"""
    try:
        print("\n" + "="*80)
        print("🚀 Processing Gap Assessment Report")
        print("="*80)
        ...
        try:
            # Save files
            excel_path = os.path.join(temp_dir, 'gap_assessment.xlsx')
            ...
            loc_level = "level2"
            if loc_ws:
                loc_level = check_loc_cells(loc_ws)
                print(f"  📍 LOC Level: {loc_level}")
            
            areas_covered_text = get_areas_covered_text(loc_level)
            ...
            date_of_audit = f"{format_date_with_superscript(start_date)} to {format_date_with_superscript(end_date)}"
            audit_period = get_financial_year(start_date)
            ...
            # Insert VICS table
            if vics_ws:
                print(f"\n📊 Inserting VICS table...")
                insert_excel_table_in_document(doc, 'VIIICSSSS_Table', vics_ws, 1, 7, 1, 250)
            
            # Insert LOC table
            if loc_ws:
                print(f"\n📊 Inserting LOC table...")
                insert_excel_table_in_document(doc, 'LOOCCC_table', loc_ws, 1, 5, 1, loc_last_row)
            
            # Insert VICS annexures
            if vics_images:
                print(f"\n🖼️ Inserting VICS annexures...")
                insert_images_as_annexures(doc, 'Annnnnnnnnnnnnnneessurer', vics_images, image_paths)
            
            # Insert LOC annexures
            if loc_images:
                print(f"\n🖼️ Inserting LOC annexures...")
                insert_images_as_annexures(doc, 'LLLLOCCCC_Annexuuuuree', loc_images, image_paths)
            ...
            # Save document
            output_path = os.path.join(temp_dir, 'Gap_Assessment_Report.docx')
            ...
            return send_file(
                output_path,
                as_attachment=True,
                download_name='Gap_Assessment_Report.docx',

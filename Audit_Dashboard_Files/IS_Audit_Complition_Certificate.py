...
is_audit_certificate_bp = Blueprint('is_audit_certificate', __name__)
...
def generate_is_audit_certificate(organization_name, city, state, auditee_designation, scope,
                                   start_audit_date, end_audit_date, report_id):
    """
    Generate an IS Audit Conducted Certificate document by copying the template and replacing placeholders
    """
    try:
        # Path to the template file
        template_path = os.path.join('static', 'Formats_and_Catalog', 'IS_Audit_Conduct_Certificate.docx')
        ...
        import shutil
        certificate_filename = "IS_Audit_Certificate.docx"
...
@is_audit_certificate_bp.route('/process_is_audit_certificate', methods=['POST'])
def process_is_audit_certificate():
    """
    Process the certificate form data and generate an IS Audit Conducted Certificate
    """
    try:
        # Get form data
        ...
        certificate_path = generate_is_audit_certificate(
        ...
        return send_file(
            certificate_path,
            as_attachment=True,
            download_name="IS_Audit_Certificate.docx",
            mimetype='application/vnd.openxmlformats-officedocument.wordprocessingml.document'
        )
        
    except Exception as e:
        print(f"❌ Error processing IS audit certificate: {e}")
        traceback.print_exc()

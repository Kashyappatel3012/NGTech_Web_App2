...
# Create blueprint
combine_assets_excels_bp = Blueprint('combine_assets_excels_bp', __name__)
...
def normalize_for_matching(text):
    """Normalize text for matching - removes spaces, converts to lowercase, removes special chars"""
    import re
...
def get_worksheet_key_for_sorting(worksheet_name):
    """Extract a key from worksheet name for sorting - handles 'Review' suffix"""
    import re
    
    # Convert to lowercase and replace underscores/hyphens with spaces
    normalized = worksheet_name.lower().replace('_', ' ').replace('-', ' ')
    
    # Remove numbers and special characters from the beginning
    normalized = re.sub(r'^[0-9_\-\.\s]+', '', normalized)
    
    # Remove "Review" suffix if present (case-insensitive)
    normalized = re.sub(r'\s+review\s*$', '', normalized, flags=re.IGNORECASE)
    normalized = normalized.strip()
    
    # Now add "review" back to the end for matching
    key = f"{normalized} review"
    
    return key

def get_worksheet_sort_order():
    """Define the order for worksheets"""
    return [
        "network review",
        "data centre review",
        ...
        "core switch review",
        "router review",
        "domain controller ad review",
        "h2h audit review",
        "antivirus review",
        "atm review",
        "mail and messaging review",
        "ho win server logical review",
        "ho windows server logical review",
        "linux server logical review",
        "esxi server logical review",
        "access control os level review",
        "access control application review",
        "application review",
        "internet banking review",
        "internal control evaluation review",
        "fire protection review",
        "amc review",
        "data input control review",
        "data input controls review",
        "purging of data files review",
        "business continuity planning review",
        "in-house and outsourced review",
        "in house and outsourced review",
        "audit trail review",
        "packaged software review",
        "user account maintenance review",
        "logical access controls review",
        "database controls review",
        "penetration testing review",
        "training review",
        "remote access review",
        "power supply ups review",
        "power supply review",
        "backup and restoration review",
        "maintenance and app patches review",
        "maintenance & application patches review",
        "network monitor tool review",
        "network monitoring tool review",
        "san switch cisco review",
        "san storage review",
        "nas review",
        "load balancer array review",
        "pam review",
        "soc review",
...
def sort_excel_files_by_worksheet_order(excel_files):
    """Sort Excel files according to the predefined worksheet order"""
    sort_order = get_worksheet_sort_order()
    
    def get_sort_key(excel_file):
        filename = excel_file['filename']
        # Extract worksheet name using the same logic as in the main function
        sheet_name = filename.replace('.xlsx', '').replace('.xls', '')
        
        # Remove everything after "Evidence" (case-insensitive) including "Evidence" itself
        evidence_index = -1
        sheet_lower = sheet_name.lower()
        
        evidence_pos = sheet_lower.find('_evidence_')
        if evidence_pos != -1:
            evidence_index = evidence_pos
        else:
            if sheet_lower.endswith('_evidence'):
                evidence_pos = sheet_lower.rfind('_evidence')
                evidence_index = evidence_pos
            elif sheet_lower.startswith('evidence_'):
                evidence_index = 0
            elif '_evidence' in sheet_lower:
                evidence_pos = sheet_lower.rfind('_evidence')
                evidence_index = evidence_pos
        
        if evidence_index != -1:
            sheet_name = sheet_name[:evidence_index]
        
        # Replace underscores with spaces
        sheet_name = sheet_name.replace('_', ' ')
        
        # Special handling: If filename ends with exactly "Application Review" (case-insensitive)
        # but NOT "Access Control Application Review", normalize it to match "application review"
        sheet_name_lower = sheet_name.lower().strip()
        
        # Check if it ends with "application review" but doesn't start with "access control"
        if sheet_name_lower.endswith('application review'):
            # Check if it's NOT "access control application review"
            # by checking if removing "application review" from the end doesn't match "access control"
            base_name = sheet_name_lower[:-len('application review')].strip()
            if base_name != 'access control':
                # This is a filename like "CTS Application Review", "CBS Application Review", etc.
                # Normalize it to "application review" for matching
                sheet_name = "application review"
        
        # Get the key for sorting (handles "Review" suffix)
        key = get_worksheet_key_for_sorting(sheet_name)
        
        # Normalize key for matching (remove spaces, lowercase, etc.)
        key_normalized = normalize_for_matching(key)
        
        # Find the index in sort order - try exact match first (case and space insensitive)
        best_match_idx = len(sort_order)
        best_match_score = 0
        
        for idx, sort_item in enumerate(sort_order):
            # Normalize sort item for matching (remove spaces, lowercase, etc.)
            sort_item_normalized = normalize_for_matching(sort_item)
            
            # Exact match after normalization (space and case insensitive)
            if key_normalized == sort_item_normalized:
                return idx
            
            # Check first 10 characters match (space and case insensitive)
            key_prefix = key_normalized[:10] if len(key_normalized) >= 10 else key_normalized
            sort_prefix = sort_item_normalized[:10] if len(sort_item_normalized) >= 10 else sort_item_normalized
            
            if key_prefix and sort_prefix and key_prefix == sort_prefix:
                # Use the first match found (according to order)
                if best_match_idx == len(sort_order):
                    best_match_idx = idx
                    best_match_score = len(key_prefix)
                # If we have a longer prefix match, prefer it
                elif len(key_prefix) > best_match_score:
                    best_match_idx = idx
                    best_match_score = len(key_prefix)
            
            # Also check if normalized strings contain each other (for partial matches)
            if key_normalized in sort_item_normalized or sort_item_normalized in key_normalized:
                # Prefer longer matches
                match_length = min(len(key_normalized), len(sort_item_normalized))
                if match_length > best_match_score:
                    best_match_score = match_length
                    best_match_idx = idx
        
        # Return the best match found, or put at the end if no match
        return best_match_idx
    
    return sorted(excel_files, key=get_sort_key)

def combine_excel_files_to_bytes(excel_files):
    """Combine all Excel files into one workbook and return as bytes"""
    try:
        # Sort Excel files according to predefined order
        sorted_excel_files = sort_excel_files_by_worksheet_order(excel_files)
        
        print("="*60)
        print("SORTED WORKSHEETS ORDER:")
        print("="*60)
        for i, excel_file in enumerate(sorted_excel_files, 1):
            filename = excel_file['filename']
            sheet_name = filename.replace('.xlsx', '').replace('.xls', '')
            
            # Apply the same naming logic
            evidence_index = -1
            sheet_lower = sheet_name.lower()
        ...
        for excel_file in sorted_excel_files:
            ...
            # Look for '_evidence_' pattern (most common case)
            evidence_pos = sheet_lower.find('_evidence_')
            if evidence_pos != -1:
                evidence_index = evidence_pos  # Remove the underscore and everything after it
            else:
                # Check if it ends with '_evidence' (no underscore after)
                if sheet_lower.endswith('_evidence'):
                    evidence_pos = sheet_lower.rfind('_evidence')
                    evidence_index = evidence_pos  # Remove the underscore and everything after it
                # Check if it starts with 'evidence_' (no underscore before)
                elif sheet_lower.startswith('evidence_'):
                    evidence_index = 0  # Remove 'evidence_' and everything after it
                # Check for standalone 'evidence' surrounded by underscores
                elif '_evidence' in sheet_lower:
                    evidence_pos = sheet_lower.rfind('_evidence')
                    evidence_index = evidence_pos  # Remove the underscore and everything after it
            ...
            sheet_name = sheet_name.strip()  # Remove leading/trailing spaces
        ...
        try:
            ...
            combined_wb.properties.title = "Combined Asset Review Excel Files"
            combined_wb.properties.creator = "Audit Dashboard"
            combined_wb.properties.subject = "Asset Review Excel Files"
            combined_wb.properties.description = "Combined Asset Review Excel files from ZIP archive"
...
@combine_assets_excels_bp.route('/test_combine_assets_excels_route')
def test_combine_assets_excels_route():
    return "Combine Assets Excels route is working!"

@combine_assets_excels_bp.route('/combine_assets_excels', methods=['POST'])
@login_required
def combine_assets_excels():
    print("="*50)
    print("COMBINE ASSETS EXCELS FORM SUBMISSION RECEIVED!")
    print("="*50)
        ...
        try:
            ...
            if extracted_files:
                ...
                if excel_files:
                    ...
                    output_filename = "Asset_Review_Excel_Files_Combined.xlsx"

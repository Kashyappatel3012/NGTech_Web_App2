...
# Create blueprint for First Audit MetaData
first_audit_metadata_bp = Blueprint('first_audit_metadata', __name__)

def create_first_audit_metadata_excel(form_data):
    ...
    try:
        ...
        # Check if value is "NA" - if so, use it as is; otherwise format as date
        if last_audit_date and last_audit_date.upper() != 'NA':
            try:
                date_obj = datetime.strptime(last_audit_date, '%Y-%m-%d')
                last_audit_date = date_obj.strftime('%d/%m/%Y')
            except:
                pass
        elif not last_audit_date:
            last_audit_date = 'NA'  # Default to NA if empty
        ...
        # Check if 0 vulnerability
        if vulnerability_status == '0':
            # Merge cells A4:K4 and add note
            ws_security.merge_cells('A4:K4')
            ws_security.row_dimensions[4].height = 30
            ws_security['A4'].value = "Note: All previously reported vulnerabilities were verified as closed during the follow-up audit."
            ws_security['A4'].font = Font(name='Arial', size=12, bold=True, color='FF008000')  # Green color
            ...
            print(f"  ✅ Created '3. Security Issues' worksheet with 0 vulnerability note")
        else:
        ...
        filename = f'First_Audit_MetaData.xlsx'
...
@first_audit_metadata_bp.route('/process_first_audit_metadata', methods=['POST'])
def process_first_audit_metadata():
    """Process First Audit MetaData form submission"""
    try:
        print("\n" + "="*80)
        ...
        # Create Excel file
        filepath, filename = create_first_audit_metadata_excel(form_data)
...
# Cleanup endpoint
@first_audit_metadata_bp.route('/cleanup_first_audit_metadata', methods=['POST'])
def cleanup_first_audit_metadata():
    """Clean up old First Audit MetaData Excel files"""
    try:
        uploads_dir = os.path.join('static', 'uploads')
        if os.path.exists(uploads_dir):
            for filename in os.listdir(uploads_dir):
                if filename.startswith('First_Audit_MetaData_') and filename.endswith('.xlsx'):

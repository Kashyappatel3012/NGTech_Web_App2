...
# Create blueprint for iOS Mobile Application Follow Up Audit MetaData
ios_application_follow_up_audit_metadata_bp = Blueprint('ios_application_follow_up_audit_metadata', __name__)

def create_ios_application_follow_up_audit_metadata_excel(form_data):
    """Create Follow Up Audit MetaData Excel file"""
    try:
        ...
        filename = f'IOS_Application_Follow_Up_Audit_MetaData_{timestamp}.xlsx'
...
@ios_application_follow_up_audit_metadata_bp.route('/process_ios_application_follow_up_audit_metadata', methods=['POST'])
def process_ios_application_follow_up_audit_metadata():
    """Process iOS Mobile Application Follow Up Audit MetaData form submission"""
    try:
        print("\n" + "="*80)
        print("🚀 Processing Follow Up Audit MetaData")
        print("="*80)
        ...
        # Create Excel file
        filepath, filename = create_ios_application_follow_up_audit_metadata_excel(form_data)
        ...
        logger.error(f"Error processing iOS application follow up audit metadata: {type(e).__name__}: {str(e)}", exc_info=True)
        # Return safe error message to client
        return jsonify({'success': False, 'error': 'An error occurred processing the request. Please try again later.'}), 500

# Cleanup endpoint
@ios_application_follow_up_audit_metadata_bp.route('/cleanup_ios_application_follow_up_audit_metadata', methods=['POST'])
def cleanup_ios_application_follow_up_audit_metadata():
    """Clean up old iOS Mobile Application Follow Up Audit MetaData Excel files"""
    try:
        uploads_dir = os.path.join('static', 'uploads')
        if os.path.exists(uploads_dir):
            for filename in os.listdir(uploads_dir):
                if filename.startswith('IOS_Application_Follow_Up_Audit_MetaData_') and filename.endswith('.xlsx'):

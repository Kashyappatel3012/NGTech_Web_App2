...
# Create blueprint for Website VAPT Follow Up Audit MetaData
website_vapt_follow_up_audit_metadata_bp = Blueprint('website_vapt_follow_up_audit_metadata', __name__)

def create_website_follow_up_audit_metadata_excel(form_data):
    ...
    try:
        ...
        filename = 'Website_Follow_Up_Audit_MetaData.xlsx'
...
@website_vapt_follow_up_audit_metadata_bp.route('/process_website_vapt_follow_up_audit_metadata', methods=['POST'])
def process_website_vapt_follow_up_audit_metadata():
    """Process Website VAPT Follow Up Audit MetaData form submission"""
    try:
        print("\n" + "="*80)
        ...
        # Create Excel file
        filepath, filename = create_website_follow_up_audit_metadata_excel(form_data)
        ...
        logger.error(f"Error processing website VAPT follow up audit metadata: {type(e).__name__}: {str(e)}", exc_info=True)
        # Return safe error message to client
        return jsonify({'success': False, 'error': 'An error occurred processing the request. Please try again later.'}), 500

# Cleanup endpoint
@website_vapt_follow_up_audit_metadata_bp.route('/cleanup_website_vapt_follow_up_audit_metadata', methods=['POST'])
def cleanup_website_vapt_follow_up_audit_metadata():
    """Clean up old Website VAPT Follow Up Audit MetaData Excel files"""
    try:
        uploads_dir = os.path.join('static', 'uploads')
        if os.path.exists(uploads_dir):
            target_file = 'Website_Follow_Up_Audit_MetaData.xlsx'

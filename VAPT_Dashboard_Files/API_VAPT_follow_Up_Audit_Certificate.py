...
# Create blueprint for API VAPT Follow Up Audit Certificate generation
api_vapt_follow_up_audit_certificate_bp = Blueprint('api_vapt_follow_up_audit_certificate', __name__)
...
def generate_api_vapt_follow_up_audit_certificate(organization_name, city, state, auditee_designation, scope,
                                       start_audit_date, end_audit_date, first_report_id, report_id):
    """
    Generate an API VAPT Follow Up Audit Completion Certificate document by copying the template and replacing placeholders
    """
    try:
        # Path to the template file
        template_path = os.path.join('static', 'Formats_and_Catalog', 'API_VAPT_Follow_Up_Audit_Conduct_Certificate.docx')
        ...
        import shutil
        certificate_filename = "API_VAPT_Follow_Up_Certificate.docx"
        certificate_path = os.path.join(temp_dir, certificate_filename)
        shutil.copy2(template_path, certificate_path)
        
        print(f"✅ API certificate template copied successfully: {certificate_path}")
        ...
        replacements = {
            ...
            "XXXXXXXXX": first_report_id  # Replace XXXXXXXXX with First Audit Report ID
        }
        
        # Perform text replacements
        print("🔄 Replacing placeholders in API certificate...")
        replace_text_in_document(doc, replacements)
        
        # Save the updated document
        doc.save(certificate_path)
        
        print(f"✅ API certificate generated successfully with replaced data: {certificate_path}")
        print(f"📋 API Certificate Data Applied:")
        ...
        print(f"❌ Error generating API certificate: {e}")
        traceback.print_exc()
        raise e

@api_vapt_follow_up_audit_certificate_bp.route('/process_api_vapt_follow_up_audit_certificate', methods=['POST'])
def process_api_vapt_follow_up_audit_certificate():
    """
    Process the certificate form data and generate an API VAPT Follow Up Audit Completion Certificate
    """
    try:
        # Get form data
        ...
        print(f"📋 API Certificate Data Received:")
        print(f"   Organization: {organization_name}")
        print(f"   City: {city}")
        print(f"   State: {state}")
        print(f"   Auditee Designation: {auditee_designation}")
        ...
        certificate_path = generate_api_vapt_follow_up_audit_certificate(
        ...
        return send_file(
            certificate_path,
            as_attachment=True,
            download_name="API_VAPT_Follow_Up_Certificate.docx",
            mimetype='application/vnd.openxmlformats-officedocument.wordprocessingml.document'
        )
        
    except Exception as e:
        print(f"❌ Error processing API VAPT follow up audit certificate: {e}")
        traceback.print_exc()

...
# Create blueprint for API Follow Up Audit MetaData
api_follow_up_audit_metadata_bp = Blueprint('api_follow_up_audit_metadata', __name__)

def create_api_follow_up_audit_metadata_excel(form_data):
    """Create API Follow Up Audit MetaData Excel file"""
    try:
        ...
        filename = f'API_Follow_Up_Audit_MetaData_{timestamp}.xlsx'
...
def get_api_follow_up_vulnerability_details():
...
def get_api_follow_up_employee_details():
...
@api_follow_up_audit_metadata_bp.route('/process_api_follow_up_audit_metadata', methods=['POST'])
def process_api_follow_up_audit_metadata():
    """Process API Follow Up Audit MetaData form submission"""
    try:
        print("\n" + "="*80)
        print("🚀 Processing API Follow Up Audit MetaData")
        print("="*80)
        ...
        form_data['vulnerabilities'] = get_api_follow_up_vulnerability_details()
        print(f"🔍 DEBUG: Vulnerabilities collected: {len(form_data['vulnerabilities'])}")
        
        print(f"🔍 DEBUG: Collecting employee details...")
        form_data['employees'] = get_api_follow_up_employee_details()
        ...
        # Create Excel file
        filepath, filename = create_api_follow_up_audit_metadata_excel(form_data)
        ...
        logger.error(f"Error processing API follow up audit metadata: {type(e).__name__}: {str(e)}", exc_info=True)
        # Return safe error message to client
        return jsonify({'success': False, 'error': 'An error occurred processing the request. Please try again later.'}), 500

# Cleanup endpoint
@api_follow_up_audit_metadata_bp.route('/cleanup_api_follow_up_audit_metadata', methods=['POST'])
def cleanup_api_follow_up_audit_metadata():
    """Clean up old VAPT API Follow Up Audit MetaData Excel files"""
    try:
        uploads_dir = os.path.join('static', 'uploads')
        if os.path.exists(uploads_dir):
            for filename in os.listdir(uploads_dir):
                if filename.startswith('API_Follow_Up_Audit_MetaData_') and filename.endswith('.xlsx'):

...
vapt_follow_up_audit_metadata_bp = Blueprint('vapt_follow_up_audit_metadata', __name__)
...
def create_follow_up_audit_metadata_excel(form_data):
    ...
    try:
        ...
        filename = 'Follow_Up_Audit_MetaData.xlsx'
...
@vapt_follow_up_audit_metadata_bp.route('/process_vapt_follow_up_audit_metadata', methods=['POST'])
def process_vapt_follow_up_audit_metadata():
    ...
    try:
        ...
        logger.error(f"Error processing VAPT follow up audit metadata: {type(e).__name__}: {str(e)}", exc_info=True)
        # Return safe error message to client
        return jsonify({'success': False, 'error': 'An error occurred processing the request. Please try again later.'}), 500

# Cleanup endpoint
@vapt_follow_up_audit_metadata_bp.route('/cleanup_vapt_follow_up_audit_metadata', methods=['POST'])
def cleanup_vapt_follow_up_audit_metadata():
    """Clean up old VAPT Follow Up Audit MetaData Excel files"""
    try:
        uploads_dir = os.path.join('static', 'uploads')
        if os.path.exists(uploads_dir):
            for filename in os.listdir(uploads_dir):
                # Clean up both old timestamped files and the new fixed filename
                if (filename.startswith('Follow_Up_Audit_MetaData_') and filename.endswith('.xlsx')) or filename == 'Follow_Up_Audit_MetaData.xlsx':

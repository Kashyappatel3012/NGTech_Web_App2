...
# Create a Blueprint for VAPT nmap routes
vapt_nmap_bp = Blueprint('vapt_nmap', __name__)
...
def generate_dynamic_filename(organization, end_date):
    ...
    """
    return "Infrastructure_VAPT_First_Audit_Report.xlsx"
        

@vapt_nmap_bp.route('/check_vapt_vulnerabilities', methods=['POST'])
def check_vapt_vulnerabilities():
    ...
    try:
        ...
        if nessus_dataframes:
            ...
            session['vapt_vulnerability_merge_state'] = {
...
@vapt_nmap_bp.route('/submit_vapt_vulnerability_details', methods=['POST'])
def submit_vulnerability_details():
    ...
    try:
        ...
        session['vapt_unmatched_vulnerability_details'] = vulnerability_details
...
@vapt_nmap_bp.route('/merge_vapt_with_matched', methods=['POST'])
def merge_vapt_with_matched():
    ...
    try:
        ...
        merge_state = session.get('vapt_vulnerability_merge_state', {})
        if not merge_state:
        ...
        # Update session
        session['vapt_vulnerability_merge_state'] = merge_state
...
@vapt_nmap_bp.route('/merge_vapt_with_unmatched', methods=['POST'])
def merge_vapt_with_unmatched():
...
@vapt_nmap_bp.route('/add_vapt_vulnerability_details', methods=['POST'])
def add_vapt_vulnerability_details():
...
@vapt_nmap_bp.route('/merge_vapt_matched_groups', methods=['POST'])
def merge_vapt_matched_groups():
...
@vapt_nmap_bp.route('/undo_vapt_last_merge', methods=['POST'])
def undo_vapt_last_merge():
...
@vapt_nmap_bp.route('/process_vapt_first_audit_report', methods=['POST'])
def process_vapt_nmap_files():
    if 'nmapFiles' not in request.files or 'nessusFiles' not in request.files:
        ...
        if nessus_dataframes:
            ...
            try:
                ...
                if 'unmatched_vulnerability_details' in session:
                    session_details = session.get('vapt_unmatched_vulnerability_details', {})
...
def create_infra_vapt_worksheet(workbook, combined_nessus, header_format, cell_format, unmatched_details_mapping=None, evidence_files=None):
    ...
    try:
        # Check if required columns exist (but still create worksheet even if missing)
        ...
        if missing_cols:
            print(f"⚠️ Missing columns in data: {missing_cols} - will create worksheet with headers only")
            # Will still create worksheet below
        # Get merge state from session
        merge_state = session.get('vapt_vulnerability_merge_state', None)
        ...
        if catalog_df is None or 'Vulnerabilities in this group' not in catalog_df.columns:
            ...
            # Set column widths even if no catalog
            worksheet_infra.set_column(0, 0, 8)  # Sr.No
            worksheet_infra.set_column(1, 1, 40)  # Vulnerabilities
            for i in range(2, len(headers)):
                if i < len(headers) - 7:  # Before POC columns
        ...
        # Expected columns: Sr.No, Vulnerabilities, Risk Factor, CVE/CWE ID, CVSS, Audit Observation, Impact, 
...
def create_summary_worksheet(workbook, combined_nessus, header_format, cell_format):
    ...
    try:
        ...
        # Column widths were already set at the beginning
        ...
        with zipfile.ZipFile(BytesIO(zip_data), 'r') as zip_ref:
            ...
            for file_name in nessus_files:
                try:
                    ...
                    device_types = ["Systems", "System", "Server", "Servers", "Switch", "Router", 'Firewall', 'Workstations', 'Access Points', 'CCTV']

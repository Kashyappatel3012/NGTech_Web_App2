...
api_first_audit_word_report_bp = Blueprint('api_first_audit_word_report', __name__)


def _extract_api_asset_hash_values(excel_file_path):
...
def _replace_application_type_placeholder(doc: Document, excel_file_path):
    """Replace the 'Innnnnternal' placeholder with the application type from Meta_Data!B14."""
    placeholder = "Innnnnternal"
    replacement_value = None

    try:
        wb = load_workbook(excel_file_path, data_only=True)
        if 'Meta_Data' not in wb.sheetnames:
            print("⚠️ 'Meta_Data' worksheet not found while replacing application type placeholder.")
            wb.close()
            return

        ws = wb['Meta_Data']
        try:
            cell_value = ws['B14'].value
            if cell_value is not None:
                replacement_value = clean_value(cell_value)
        except Exception as e:
            print(f"⚠️ Error reading Meta_Data!B14 for application type: {e}")

        wb.close()
    except Exception as e:
        print(f"⚠️ Error loading workbook for application type replacement: {e}")
        traceback.print_exc()

    if not replacement_value:
        print("⚠️ No application type value found in Meta_Data!B14; skipping replacement.")
        return

    def process_paragraph(paragraph):
        while placeholder in paragraph.text:
            _replace_text_in_runs(paragraph, placeholder, replacement_value)
...
def _prepare_api_excel_for_web_logic(original_excel_path):
    """
    Create a temporary copy of the API Excel file with worksheet names tailored for
    ...
    try:
        ...
        if 'API VAPT' in wb.sheetnames and 'Web Application VAPT' not in wb.sheetnames:
            ws = wb['API VAPT']
        ...
        print(f"⚠️ Error preparing API Excel for Word logic: {e}")
        traceback.print_exc()
...
def _check_empty_a2_and_handle_api(doc, original_excel_file):
    """Check if A2 is empty in API VAPT worksheet and handle accordingly"""
    try:
        wb = load_workbook(original_excel_file, data_only=True)
        if 'API VAPT' in wb.sheetnames:
            ws = wb['API VAPT']
...
def _check_empty_a2_for_a11_api(doc, original_excel_file):
    """Check if A2 is empty in API VAPT worksheet for a11 function"""
    try:
        df = pd.read_excel(original_excel_file, sheet_name='API VAPT')
...
def generate_api_word_report_from_excel(excel_file_path):
    """
    Generate API VAPT First Audit Word report by leveraging the
    Web Application Word report logic and applying API-specific placeholder replacements.
    """
    temp_excel_for_web = _prepare_api_excel_for_web_logic(excel_file_path)

    try:
        template_path = os.path.join('static', 'Formats_and_Catalog', 'API_VAPT_First_Audit_Report.docx')
        ...
        if _check_empty_a2_and_handle_api(doc, excel_file_path):
        ...
        if not _check_empty_a2_for_a11_api(doc, excel_file_path):
        ...
        hash_values = _extract_api_asset_hash_values(excel_file_path)
        _replace_hash_placeholders_in_doc(doc, hash_values)
        _replace_application_type_placeholder(doc, excel_file_path)

        print("✅ API VAPT Word report generated successfully from Excel file")
        return doc
...
@api_first_audit_word_report_bp.route('/process_api_vapt_first_audit_word_report', methods=['POST'])
def process_api_first_audit_word_report():
    """Process the uploaded Excel file and generate an API VAPT Word report."""
    try:
        ...
        try:
            print("🔍 Starting API VAPT Word report generation...")
            doc = generate_api_word_report_from_excel(temp_excel_path)
            ...
            response = send_file(
                temp_doc_path,
                as_attachment=True,
                download_name='API_VAPT_First_Audit_Report.docx',
            ...
            print("✅ API VAPT Word report generated successfully.")
            return response

        except Exception as processing_error:
            print(f"❌ Error generating API Word report: {processing_error}")
            traceback.print_exc()
        ...
        print(f"❌ Error in API Word report route: {e}")
        traceback.print_exc()

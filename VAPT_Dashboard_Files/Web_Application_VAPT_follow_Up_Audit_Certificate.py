...
# Create blueprint for Web Application VAPT Follow Up Audit Certificate generation
web_app_vapt_follow_up_audit_certificate_bp = Blueprint('web_app_vapt_follow_up_audit_certificate', __name__)
...
def generate_web_app_vapt_follow_up_audit_certificate(organization_name, city, state, auditee_designation, scope,
                                                      start_audit_date, end_audit_date, first_report_id, report_id):
    """Generate a Web Application VAPT Follow Up Audit Completion Certificate document."""
    try:
        template_path = os.path.join('static', 'Formats_and_Catalog', 'Web_Application_VAPT_Follow_Up_Audit_Conduct_Certificate.docx')
        ...
        import shutil
        certificate_filename = "Web_Application_VAPT_Follow_Up_Certificate.docx"
        certificate_path = os.path.join(temp_dir, certificate_filename)
        shutil.copy2(template_path, certificate_path)

        print(f"✅ Web Application certificate template copied successfully: {certificate_path}")
        ...
        print("🔄 Replacing placeholders in Web Application certificate...")
        replace_text_in_document(doc, replacements)

        doc.save(certificate_path)

        print(f"✅ Web Application certificate generated successfully with replaced data: {certificate_path}")
        print("📋 Web Application Certificate Data Applied:")
        ...
        print(f"❌ Error generating Web Application certificate: {e}")
        traceback.print_exc()
        raise e


@web_app_vapt_follow_up_audit_certificate_bp.route('/process_web_app_vapt_follow_up_audit_certificate', methods=['POST'])
def process_web_app_vapt_follow_up_audit_certificate():
    """Process the form data and generate a Web Application VAPT Follow Up Audit Completion Certificate."""
    try:
        ...
        print("📋 Web Application Follow Up Certificate Data Received:")
        print(f"   Organization: {organization_name}")
        print(f"   City: {city}")
        print(f"   State: {state}")
        print(f"   Auditee Designation: {auditee_designation}")
        ...
        certificate_path = generate_web_app_vapt_follow_up_audit_certificate(
        ...
        return send_file(
            certificate_path,
            as_attachment=True,
            download_name="Web_Application_VAPT_Follow_Up_Certificate.docx",
            mimetype='application/vnd.openxmlformats-officedocument.wordprocessingml.document'
        )

    except Exception as e:
        print(f"❌ Error processing Web Application follow up audit certificate: {e}")
        traceback.print_exc()

...
# Create blueprint for Android Mobile Application VAPT First Audit MetaData
android_application_first_audit_metadata_bp = Blueprint('android_application_first_audit_metadata', __name__)

def create_android_application_first_audit_metadata_excel(form_data):
    """Create First Audit MetaData Excel file"""
    try:
        ...
        for cell_ref, value in headers.items():
            cell = ws[cell_ref]
            # Sanitize value to prevent Excel injection
        ...
        from excel_security_utils import sanitize_excel_value
        ws_audits['C4'].value = sanitize_excel_value(form_data.get('categoryOfOrg', ''))
        ...
        ws_audits['D4'].value = sanitize_excel_value(form_data.get('sectorOfOrg', ''))
        ...
        # E4 = Sub Sector
        ws_audits['E4'].value = sanitize_excel_value(form_data.get('subSector', ''))
        ...
        if type_of_audit and 'other' in type_of_audit.lower():
            ws_audits['G4'].value = sanitize_excel_value(form_data.get('typeOfAuditOther', ''))
        else:
        ...
        ws_audits['H4'].value = sanitize_excel_value(form_data.get('detailsOfWebsite', ''))
        ...
        ws_audits['I4'].value = sanitize_excel_value(form_data.get('reasonForAudit', ''))
        ...
        filename = 'Android_Application_First_Audit_MetaData.xlsx'
...
@android_application_first_audit_metadata_bp.route('/process_android_application_vapt_first_audit_metadata', methods=['POST'])
def process_android_application_vapt_first_audit_metadata():
    """Process Android Mobile Application VAPT First Audit MetaData form submission"""
    try:
        print("\n" + "="*80)
        print("🚀 Processing First Audit MetaData")
        print("="*80)
        ...
        # Create Excel file
        filepath, filename = create_android_application_first_audit_metadata_excel(form_data)
        ...
        logger.error(f"Error processing Android application first audit metadata: {type(e).__name__}: {str(e)}", exc_info=True)
        # Return safe error message to client
        return jsonify({'success': False, 'error': 'An error occurred processing the request. Please try again later.'}), 500

# Cleanup endpoint
@android_application_first_audit_metadata_bp.route('/cleanup_android_application_vapt_first_audit_metadata', methods=['POST'])
def cleanup_android_application_vapt_first_audit_metadata():
    """Clean up old Android Mobile Application VAPT First Audit MetaData Excel files"""
    try:
        uploads_dir = os.path.join('static', 'uploads')
        if os.path.exists(uploads_dir):
            target_file = 'Android_Application_First_Audit_MetaData.xlsx'

...
# Create blueprint for API First Audit MetaData
api_first_audit_metadata_bp = Blueprint('api_first_audit_metadata', __name__)

def create_api_first_audit_metadata_excel(form_data):
    ...
    try:
        ...
        filename = f'API_First_Audit_MetaData_{timestamp}.xlsx'
...
def get_api_vulnerability_details():
...
@api_first_audit_metadata_bp.route('/process_api_first_audit_metadata', methods=['POST'])
def process_api_first_audit_metadata():
    """Process VAPT First Audit MetaData form submission"""
    try:
        print("\n" + "="*80)
        print("🚀 Processing API First Audit MetaData")
        print("="*80)
        ...
        # Create Excel file
        filepath, filename = create_api_first_audit_metadata_excel(form_data)
...
# Cleanup endpoint
@api_first_audit_metadata_bp.route('/cleanup_api_first_audit_metadata', methods=['POST'])
def cleanup_api_first_audit_metadata():
    """Clean up old VAPT First Audit MetaData Excel files"""
    try:
        uploads_dir = os.path.join('static', 'uploads')
        if os.path.exists(uploads_dir):
            for filename in os.listdir(uploads_dir):
                if filename.startswith('API_First_Audit_MetaData_') and filename.endswith('.xlsx'):

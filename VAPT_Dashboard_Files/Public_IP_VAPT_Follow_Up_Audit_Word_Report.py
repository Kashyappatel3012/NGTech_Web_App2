...
public_ip_vapt_follow_up_word_report_bp = Blueprint('public_ip_vapt_follow_up_word_report', __name__)
        ...
        for row in sheet.iter_rows():
            for cell in row:
                if cell.value is not None:
                    ...
                    # Special handling for "ORGANIZATION EMAIL ADDRESSES" section (for Website VAPT)
...
def read_scope_worksheet_data(excel_file):
    ...
    Returns host values from column B
    
    Args:
        excel_file (str): Path to Excel file
        
    Returns:
        list: List of dictionaries containing host values from column B
    """
    try:
        # Load Excel workbook
        wb = load_workbook(excel_file)
...
def replace_scope_placeholders_in_table(table, target_row_idx, target_cell_idx, scope_data):
    ...
    Uses host values from column B only
    """
    try:
...
def replace_followup_vulnerability_details_with_images(doc, excel_file):
    ...
    try:
        ...
        for idx, row in enumerate(df.iterrows(), start=1):
            ...
            # Get data from column index 9 (Excel column J, index 9) - same as line 2362
...
def create_nmap_table_from_excel(doc, excel_file):
    ...
    try:
        ...
        # For Public IP VAPT, use columns A-F (6 columns)
        ...
        for merged_range in ws.merged_cells:
            ...
            # Skip if merged cell is outside columns A-F
            if start_col >= 6:
                continue
            
            bottom_right_cell_ref = merged_range_coords[-1]
            end_row = bottom_right_cell_ref[0] - 1
            end_col = min(bottom_right_cell_ref[1] - 1, 5)  # Limit to column F
        ...
        for i, row in enumerate(ws.iter_rows(values_only=False)):
            ...
            for j, cell in enumerate(row[:6]):
                ...
                # Format font - force Calibri for OPEN_PORTSSSS table
                if cell.font:
                    ...
                    run.font.name = "Calibri"  # Force Calibri font for OPEN_PORTSSSS table
                    run.font.size = Pt(12)
                    ...
                    # If no font from Excel, use Calibri
...
def replace_a11_row_with_vulnerabilities(doc, excel_file):
    ...
    try:
        ...
        for idx in range(len(df)):
            ...
            # Column 2: Affected System (Excel column J, index 9) - Affected Systems
            ...
            # Column 3: Vulnerability Description (Excel column C, index 2) - Name of Vulnerability
            ...
            # Column 6: Audit Observation - Set to "NA"
            ...
            # Column 11: Status from Excel (Excel column L, index 11)
...
def generate_follow_up_audit_word_report(excel_file_path):
    ...
    try:
        ...
        template_path = os.path.join("static", "Formats_and_Catalog", "Public_IP_VAPT_Follow_Up_Audit_Report.docx")
        ...
        temp_dir = tempfile.mkdtemp()
        temp_file_path = os.path.join(temp_dir, "Public_IP_VAPT_Follow_Up_Audit_Report.docx")
...
@public_ip_vapt_follow_up_word_report_bp.route('/process_public_ip_vapt_follow_up_audit_word_report', methods=['POST'])
def process_public_ip_follow_up_audit_word_report():
    """
    Process the uploaded Excel file and generate a Follow-Up Audit Word report
    """
    try:
        # Check if file was uploaded
        if 'publicIpFollowUpExcelFile' not in request.files:
            return jsonify({'error': 'No file uploaded'}), 400
        
        file = request.files['publicIpFollowUpExcelFile']
        ...
        try:
            ...
            return send_file(
                word_file_path,
                as_attachment=True,
                download_name='Public_IP_VAPT_Follow_Up_Audit_Report.docx',

...
# Create blueprint for Public IP VAPT Follow Up Audit Certificate generation
public_ip_vapt_follow_up_audit_certificate_bp = Blueprint('public_ip_vapt_follow_up_audit_certificate', __name__)
...
def generate_public_ip_vapt_follow_up_audit_certificate(organization_name, city, state, auditee_designation, scope,
                                                        start_audit_date, end_audit_date, first_report_id, report_id):
    """
    Generate a Public IP VAPT Follow Up Audit Completion Certificate document by copying the template and replacing placeholders
    """
    try:
        template_path = os.path.join('static', 'Formats_and_Catalog', 'Public_IP_VAPT_Follow_Up_Audit_Conduct_Certificate.docx')
        ...
        import shutil
        certificate_filename = "Public_IP_VAPT_Follow_Up_Certificate.docx"
        certificate_path = os.path.join(temp_dir, certificate_filename)
        shutil.copy2(template_path, certificate_path)

        print(f"✅ Public IP certificate template copied successfully: {certificate_path}")
        ...
        print("🔄 Replacing placeholders in Public IP certificate...")
        replace_text_in_document(doc, replacements)

        doc.save(certificate_path)

        print(f"✅ Public IP certificate generated successfully with replaced data: {certificate_path}")
        print("📋 Public IP Certificate Data Applied:")
        ...
        print(f"❌ Error generating Public IP certificate: {e}")
        traceback.print_exc()
        raise e


@public_ip_vapt_follow_up_audit_certificate_bp.route('/process_public_ip_vapt_follow_up_audit_certificate', methods=['POST'])
def process_public_ip_vapt_follow_up_audit_certificate():
    """
    Process the certificate form data and generate a Public IP VAPT Follow Up Audit Completion Certificate
    """
    try:
        ...
        print("📋 Public IP Follow Up Certificate Data Received:")
        print(f"   Organization: {organization_name}")
        print(f"   City: {city}")
        print(f"   State: {state}")
        print(f"   Auditee Designation: {auditee_designation}")
        ...
        certificate_path = generate_public_ip_vapt_follow_up_audit_certificate(
        ...
        return send_file(
            certificate_path,
            as_attachment=True,
            download_name="Public_IP_VAPT_Follow_Up_Certificate.docx",
            mimetype='application/vnd.openxmlformats-officedocument.wordprocessingml.document'
        )

    except Exception as e:
        print(f"❌ Error processing Public IP follow up audit certificate: {e}")
        traceback.print_exc()

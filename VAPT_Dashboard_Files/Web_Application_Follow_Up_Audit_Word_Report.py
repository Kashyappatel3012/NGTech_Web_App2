...
web_app_follow_up_word_report_bp = Blueprint('web_app_follow_up_word_report', __name__)
...
def replace_scope_placeholders_with_data(doc, excel_file, form_metadata=None):
    ...
    try:
        if form_metadata and 'asset_data' in form_metadata:
            ...
            application_type = form_metadata.get('internal_external', 'Internal')
            if asset_data:
                print(f"🔍 Using {len(asset_data)} assets from form data")
                replace_scope_placeholders_with_form_assets(doc, asset_data, application_type)
            else:
...
def replace_scope_placeholders_with_form_assets(doc, asset_data, application_type='Internal'):
    """
                ...
                for cell_idx, cell in enumerate(row.cells):
                    for paragraph in cell.paragraphs:
                        if "Branch Namee or Server Namee" in paragraph.text or "IP Addressesss" in paragraph.text:
                            ...
                            replace_scope_placeholders_in_table_with_assets(table, row_idx, cell_idx, asset_data, application_type)
                            replace_asset_placeholders_globally_with_assets(doc, asset_data, application_type)
                            return
...
def _asset_placeholder_mapping_from_form(asset, asset_index, application_type='Internal'):
    """
    ...
    # Use internal_ip for Internal, external_ip for External
    ip_address = internal_ip if application_type == 'Internal' else external_ip
    
    return {
        "Branch Namee or Server Namee": asset.get('description', 'NA'),
        "IP Addressesss": ip_address,
        ...
        f"Asset {asset_index + 1} - Internal IP Addresses": internal_ip,
        f"Asset {asset_index + 1} - External IP Address": external_ip,
        f"Asset {asset_index + 1} - URL": asset.get('url', 'NA'),
        f"Asset {asset_index + 1} - Version": asset.get('version', 'NA')
    }


def _fill_asset_placeholders_in_row_from_form(row, asset, asset_index, application_type='Internal'):
    """
    Replace placeholders in a given table row with asset data from form.
    """
    replacements = _asset_placeholder_mapping_from_form(asset, asset_index, application_type)
...
def replace_scope_placeholders_in_table_with_assets(table, target_row_idx, target_cell_idx, asset_data, application_type='Internal'):
    """
    ...
    try:
        ...
        for idx, asset in enumerate(asset_data):
            ...
            _fill_asset_placeholders_in_row_from_form(row, asset, idx, application_type)
...
def replace_asset_placeholders_globally_with_assets(doc, asset_data, application_type='Internal'):
    """
    ...
    replacements = _asset_placeholder_mapping_from_form(first_asset, 0, application_type)
...
def read_scope_worksheet_data(excel_file):
    ...
    try:
        ...
        print("⚠️ Could not import Web Application scope reader, falling back to empty list.")
        return []


def replace_followup_vulnerability_details_with_images(doc, excel_file):
    """
    Build the vulnerability section for the Web Application follow-up report.
    This mirrors the follow-up logic but reads from the 'Web Application VAPT' sheet.
    """
    try:
        ...
        for sheet_name in wb.sheetnames:
            if "web application vapt" in sheet_name.lower() or "web application" in sheet_name.lower():
                ws = wb[sheet_name]
                break
        
        if not ws:
            print("⚠️ 'Web Application VAPT' worksheet not found")
            wb.close()
            return
...
def extract_followup_poc_images_from_excel(excel_file):
    """
    Extract POC images from the Web Application follow-up Excel file.
    ...
    try:
        ...
        if hasattr(ws, "_images"):
            print(f"📸 Found {len(ws._images)} images in Web Application sheet")
        ...
        # Print summary
        print(f"📊 Web extracted {len(poc_images_final)} vulnerabilities with POC (New) images")
        print(f"📊 Web extracted {len(old_poc_images)} vulnerabilities with Old POC images")
        print(f"📊 Web extracted {len(remarks_dict)} vulnerabilities with remarks")

        return poc_images_final, old_poc_images, df, remarks_dict

    except Exception as e:
        print(f"❌ Error extracting Web follow-up images: {e}")
        traceback.print_exc()
        return {}, {}, pd.DataFrame(), {}
...
def add_followup_images_at_end_of_document(doc, poc_images, old_poc_images, df):
    """
    Append old and new POC images for Web Application follow-up audit at the end of the document.
    """
    try:
        ...
        if not has_poc and not has_old_poc:
            print("⚠️ No Web POC images to append")
            return
        ...
        print(f"✅ Added {image_count} vulnerability sections with Web POC images")

    except Exception as e:
        print(f"❌ Error adding Web POC images: {e}")
        traceback.print_exc()
...
def replace_followup_poc_placeholders_with_images(doc, excel_file):
    """
    Replace POC placeholders (POCsss / New_POCCsssss) in the Web follow-up report with images.
    """
    try:
        poc_images, old_poc_images, df, remarks_dict = extract_followup_poc_images_from_excel(excel_file)
...
def get_followup_vulnerability_counts(excel_file):
    """
    Extract vulnerability counts from "Web Application VAPT" worksheet:
...
def generate_follow_up_audit_word_report(excel_file_path, form_metadata=None):
    """
    Generate Follow-Up Audit Word report for Web Application from Excel file
    form_metadata: Dictionary containing metadata from form (if provided, will be used instead of Excel Meta_Data)
    """
    try:
        template_path = os.path.join("static", "Formats_and_Catalog", "Web_Application_VAPT_Follow_Up_Audit_Report.docx")
        ...
        temp_dir = tempfile.mkdtemp()
        temp_file_path = os.path.join(temp_dir, "Web_Application_VAPT_Follow_Up_Audit_Report.docx")
...
@web_app_follow_up_word_report_bp.route('/process_web_app_follow_up_audit_word_report', methods=['POST'])
def process_web_app_follow_up_audit_word_report():
    """
    Process the uploaded Excel file and generate a Web Application Follow-Up Audit Word report
    """
    try:
        ...
        application_type = request.form.get('applicationType', 'Internal')
        ...
        asset_ips = request.form.getlist('assetInternalIp[]')
        ...
        for idx in range(len(asset_descriptions)):
            if asset_descriptions[idx] and asset_criticalities[idx] and asset_ips[idx] and asset_urls[idx] and asset_versions[idx]:
                # Use the IP field for both internal and external (form only has one IP field)
                asset_info = {
                    'description': asset_descriptions[idx],
                    'criticality': asset_criticalities[idx],
                    'internal_ip': asset_ips[idx],
                    'external_ip': asset_ips[idx],  # Same IP for both (form has single IP field)
                    'url': asset_urls[idx],
        ...
        form_metadata = {
            ...
            "first_audit_report_id": first_audit_report_id,
            "first_audit_report_date": first_audit_report_date,
            "internal_external": application_type,
        ...
        try:
            ...
            return send_file(
                word_file_path,
                as_attachment=True,
                download_name='Web_Application_VAPT_Follow_Up_Audit_Report.docx',
        ...
        print(f"❌ Error processing web application follow-up audit word report: {e}")
        traceback.print_exc()

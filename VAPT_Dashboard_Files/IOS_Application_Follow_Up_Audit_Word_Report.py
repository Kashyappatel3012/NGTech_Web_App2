...
ios_follow_up_word_report_bp = Blueprint('ios_app_follow_up_word_report', __name__)
...
def read_scope_worksheet_data(excel_file):
    ...
    try:
        ...
        print("⚠️ Could not import IOS Mobile Application scope reader, falling back to empty list.")
        return []


def replace_followup_vulnerability_details_with_images(doc, excel_file):
    """
    Build the vulnerability section for the IOS Mobile Application follow-up report.
    This mirrors the follow-up logic but reads from the 'IOS Mobile Application VAPT' sheet.
    """
    try:
        ...
        for sheet_name in wb.sheetnames:
            if "ios mobile application vapt" in sheet_name.lower() or "ios mobile application" in sheet_name.lower():
                ws = wb[sheet_name]
                break
        
        if not ws:
            print("⚠️ 'IOS Mobile Application VAPT' worksheet not found")
            wb.close()
            return
...
def extract_followup_poc_images_from_excel(excel_file):
    """
    Extract POC images from the IOS Mobile Application follow-up Excel file.
    ...
    try:
        ...
        if not ws:
            print("❌ 'IOS Mobile Application VAPT' worksheet not found")
            wb.close()
            return {}, {}, pd.DataFrame()
        ...
        if hasattr(ws, "_images"):
            print(f"📸 Found {len(ws._images)} images in IOS Mobile Application sheet")
        ...
        # Print summary
        print(f"📊 IOS extracted {len(poc_images_final)} vulnerabilities with POC (New) images")
        print(f"📊 IOS extracted {len(old_poc_images)} vulnerabilities with Old POC images")
        print(f"📊 IOS extracted {len(remarks_dict)} vulnerabilities with remarks")

        return poc_images_final, old_poc_images, df, remarks_dict

    except Exception as e:
        print(f"❌ Error extracting IOS follow-up images: {e}")
        traceback.print_exc()
        return {}, {}, pd.DataFrame(), {}
...
def add_followup_images_at_end_of_document(doc, poc_images, old_poc_images, df):
    """
    Append old and new POC images for IOS Mobile Application follow-up audit at the end of the document.
    """
    try:
        ...
        if not has_poc and not has_old_poc:
            print("⚠️ No IOS POC images to append")
            return
        ...
        print(f"✅ Added {image_count} vulnerability sections with IOS POC images")

    except Exception as e:
        print(f"❌ Error adding IOS POC images: {e}")
        traceback.print_exc()
...
def replace_followup_poc_placeholders_with_images(doc, excel_file):
    ...
    try:
        ...
        print(f"❌ Error replacing IOS follow-up POC placeholders: {e}")
        traceback.print_exc()
...
def replace_open_ports_with_nmap_table(doc, excel_file, placeholder="OPEN_PORTSSSS"):
    ...
    if not nmap_table:
        print("❌ Failed to create IOS Nmap table")
        return False
    ...
    for paragraph in doc.paragraphs:
        if placeholder in paragraph.text:
            ...
            print(f"✅ Replaced '{placeholder}' with IOS Nmap table")
            return True
            ...
            for cell in row.cells:
                for paragraph in cell.paragraphs:
                    if placeholder in paragraph.text:
                        ...
                        print(f"✅ Replaced '{placeholder}' with IOS Nmap table inside table cell")
                        return True

    print(f"⚠️ Placeholder '{placeholder}' not found in IOS document")
    return False
...
def replace_a11_row_with_vulnerabilities(doc, excel_file):
    ...
    try:
        ...
        df = pd.read_excel(excel_file, sheet_name='IOS Mobile Application VAPT')

        if df.empty:
            print("No data found in 'IOS Mobile Application VAPT' worksheet")
            return
...
def get_followup_vulnerability_counts(excel_file):
    """
    Extract vulnerability counts from "IOS Mobile Application VAPT" worksheet:
...
def generate_follow_up_audit_word_report(excel_file_path, form_metadata=None):
    """
    Generate Follow-Up Audit Word report for IOS Mobile Application from Excel file
    form_metadata: Dictionary containing metadata from form (if provided, will be used instead of Excel Meta_Data)
    """
    try:
        template_path = os.path.join("static", "Formats_and_Catalog", "IOS_Mobile_Application_VAPT_Follow_Up_Audit_Report.docx")
        ...
        temp_dir = tempfile.mkdtemp()
        temp_file_path = os.path.join(temp_dir, "IOS_Mobile_Application_VAPT_Follow_Up_Audit_Report.docx")
...
@ios_follow_up_word_report_bp.route('/process_ios_app_vapt_follow_up_audit_word_report', methods=['POST'])
def process_ios_app_vapt_follow_up_audit_word_report():
    """
    Process the uploaded Excel file and generate an IOS Mobile Application Follow-Up Audit Word report
    """
    try:
        ...
        try:
            ...
            return send_file(
                word_file_path,
                as_attachment=True,
                download_name='IOS_Mobile_Application_VAPT_Follow_Up_Audit_Report.docx',
        ...
        print(f"❌ Error processing ios application follow-up audit word report: {e}")
        traceback.print_exc()

...
public_ip_vapt_first_audit_word_report_bp = Blueprint('public_ip_vapt_first_audit_word_report', __name__)
...
def create_nmap_table_from_excel(doc, excel_file):
    ...
    try:
        ...
        for merged_range in ws.merged_cells:
            ...
            # Only process merged cells that are within first 6 columns (A-F)
            if end_col >= 6:
                continue
        ...
        # Fill Word table with Excel data and formatting (only first 6 columns)
        for i, row in enumerate(ws.iter_rows(values_only=False)):
            for j, cell in enumerate(row):
                # Only process first 6 columns (A, B, C, D, E, F)
                if j >= 6:
                    break
...
def extract_poc_images_from_excel(excel_file):
    ...
    try:
        ...
        print("Worksheet 'Public_IP_VAPT' not found in Excel file. Returning empty data.")
        return {}, pd.DataFrame()
    except Exception as e:
...
def get_vulnerability_counts(excel_file):
    ...
    try:
        ...
        if df.empty:
            print("No data found in 'Public_IP_VAPT' worksheet")
            return [], []
...
def replace_vulnerability_details_with_images(doc, excel_file):
    ...
    """
    try:
        # Check if A2 cell is empty in Public_IP_VAPT worksheet
        try:
            wb = load_workbook(excel_file, data_only=True)
            if 'Public_IP_VAPT' in wb.sheetnames:
                ws = wb['Public_IP_VAPT']
                ...
                if a2_value is None or str(a2_value).strip() == '':
                    ...
                    for paragraph in doc.paragraphs:
                        if "Tables_Heree" in paragraph.text:
                            ...
                            run = paragraph.add_run("Note: No vulnerabilities were identified during the Public IP VAPT Audit.")
                    ...
                    for paragraph in doc.paragraphs:
                        for idx, run in enumerate(paragraph.runs):
                            if "Tables_Heree" in run.text:
                                ...
                                run.text = "Note: No vulnerabilities were identified during the Public IP VAPT Audit."
                                run.font.color.rgb = RGBColor(0, 128, 0)  # Green color
                                run.font.bold = True
                                paragraph.alignment = WD_ALIGN_PARAGRAPH.CENTER
        ...
        for idx, row in enumerate(df.iterrows(), start=1):
            ...
            # Get data from column index 9 (Excel column J, index 9) - same as line 2471
...
def replace_scope_placeholders_with_data(doc, excel_file):
    """
    Find and replace 'PUUUBLIC_IPPP' placeholder with data from the Scope worksheet,
    ...
    try:
        ...
        # Find tables containing the PUUUBLIC_IPPP placeholder
        for table in doc.tables:
            for row_idx, row in enumerate(table.rows):
                for cell_idx, cell in enumerate(row.cells):
                    for paragraph in cell.paragraphs:
                        cell_text = paragraph.text
                        if "PUUUBLIC_IPPP" in cell_text:
                            print(f"🔍 Found PUUUBLIC_IPPP placeholder in row {row_idx}, cell {cell_idx}: {cell_text}")
                            # Found the target row, now replace and add new rows
                            replace_scope_placeholders_in_table(table, row_idx, cell_idx, scope_data)
                            return
        
        print("⚠️ PUUUBLIC_IPPP placeholder not found in any table")
...
def replace_scope_placeholders_in_table(table, target_row_idx, target_cell_idx, scope_data):
    ...
    try:
        ...
        if processed_data:
            ...
            # Replace PUUUBLIC_IPPP with host
            for paragraph in target_row.cells[target_cell_idx].paragraphs:
                if "PUUUBLIC_IPPP" in paragraph.text:
                    _replace_text_in_runs(paragraph, "PUUUBLIC_IPPP", first_entry['host'])
        ...
        for i in range(1, len(processed_data)):
            ...
            for j, cell in enumerate(new_row.cells):
                if j < len(target_row.cells):
                    ...
                    if j == target_cell_idx:  # PUUUBLIC_IPPP cell - replace with host
...
def replace_a11_row_with_vulnerabilities(doc, excel_file):
    ...
    try:
        ...
        if not df.empty and len(df) >= 1:
            ...
            note_message = "Note: No vulnerabilities were identified during the Public IP VAPT Audit."
...
def generate_word_report_from_excel(excel_file_path):
    ...
    try:
        ...
        template_path = os.path.join('static', 'Formats_and_Catalog', 'Public_IP_VAPT_First_Audit_Report.docx')
...
@public_ip_vapt_first_audit_word_report_bp.route('/process_public_ip_vapt_first_audit_word_report', methods=['POST'])
def process_public_ip_first_audit_word_report():
    """
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                word_temp_file_path,
                as_attachment=True,
                download_name='Public_IP_VAPT_First_Audit_Report.docx',

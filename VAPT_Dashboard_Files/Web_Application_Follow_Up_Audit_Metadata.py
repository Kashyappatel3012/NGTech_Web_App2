...
web_application_follow_up_audit_metadata_bp = Blueprint('web_application_follow_up_audit_metadata', __name__)

def create_web_application_follow_up_audit_metadata_excel(form_data):
    ...
    try:
        ...
        filename = f'Web_Application_Follow_Up_Audit_MetaData_{timestamp}.xlsx'
...
@web_application_follow_up_audit_metadata_bp.route('/process_web_application_follow_up_audit_metadata', methods=['POST'])
def process_web_application_follow_up_audit_metadata():
    ...
    try:
        ...
        # Create Excel file
        filepath, filename = create_web_application_follow_up_audit_metadata_excel(form_data)
        ...
        logger.error(f"Error processing web application follow up audit metadata: {type(e).__name__}: {str(e)}", exc_info=True)
        # Return safe error message to client
        return jsonify({'success': False, 'error': 'An error occurred processing the request. Please try again later.'}), 500

# Cleanup endpoint
@web_application_follow_up_audit_metadata_bp.route('/cleanup_web_application_follow_up_audit_metadata', methods=['POST'])
def cleanup_web_application_follow_up_audit_metadata():
    """Clean up old VAPT Follow Up Audit MetaData Excel files"""
    try:
        uploads_dir = os.path.join('static', 'uploads')
        if os.path.exists(uploads_dir):
            for filename in os.listdir(uploads_dir):
                if filename.startswith('Web_Application_Follow_Up_Audit_MetaData_') and filename.endswith('.xlsx'):

# Public IP VAPT First Audit Excel Report Generator
...
# Create a Blueprint for Public IP VAPT routes
public_ip_vapt_bp = Blueprint('public_ip_vapt', __name__)
...
def generate_public_ip_filename(organization, end_date):
    """
    Generate static filename for Public IP VAPT First Audit.
    Format: Public_IP_VAPT_First_Audit
    """
    return "Public_IP_VAPT_First_Audit_Report.xlsx"
...
@public_ip_vapt_bp.route('/check_public_ip_vapt_vulnerabilities', methods=['POST'])
def check_public_ip_vapt_vulnerabilities():
    ...
    try:
        ...
        if nessus_dataframes:
            ...
            # Store vulnerability data in session for merge operations
            # Limit sizes to prevent session overflow (like Website VAPT)
            matched_groups_limited = matched_groups[:50]  # Limit to 50 groups max
            unmatched_vulnerabilities_limited = unmatched_vulnerabilities[:100]  # Limit to 100 vulnerabilities max
            
            # Preserve existing new_group_details and manual vulnerabilities to prevent data loss
            existing_merge_state = session.get('public_ip_vapt_vulnerability_merge_state', {})
            
            session['public_ip_vapt_vulnerability_merge_state'] = {
                'matched_groups': matched_groups_limited,
                'unmatched_vulnerabilities': unmatched_vulnerabilities_limited,
                'merge_operations': existing_merge_state.get('merge_operations', [])  # Only preserve operations
            }
            
            print(f"💾 Session updated: Preserved {len(existing_merge_state.get('new_group_details', {}))} items in new_group_details")
...
@public_ip_vapt_bp.route('/merge_public_ip_vapt_with_matched', methods=['POST'])
def merge_public_ip_with_matched():
    ...
    try:
        ...
        merge_state = session.get('public_ip_vapt_vulnerability_merge_state', {})
        if not merge_state:
        ...
        # Update session
        session['public_ip_vapt_vulnerability_merge_state'] = merge_state
...
@public_ip_vapt_bp.route('/merge_public_ip_vapt_with_unmatched', methods=['POST'])
def merge_public_ip_with_unmatched():
    ...
    try:
        ...
        if 'new_group_details' not in merge_state:
            merge_state['new_group_details'] = {}
        ...
        update_public_ip_catalog_with_vulnerabilities({
            vulnerability_details['vulnerabilityName']: vulnerability_details
        })
...
@public_ip_vapt_bp.route('/merge_public_ip_vapt_matched_groups', methods=['POST'])
def merge_public_ip_matched_groups():
    ...
    try:
        ...
        for vuln in source_group.get('matched_vulnerabilities', []):
...
@public_ip_vapt_bp.route('/public_ip_vapt_undo_last_merge', methods=['POST'])
def public_ip_undo_last_merge():
...
@public_ip_vapt_bp.route('/add_public_ip_manual_vulnerability', methods=['POST'])
def add_public_ip_manual_vulnerability():
    ...
    try:
        ...
        if 'manually_added_vulnerabilities' not in merge_state:
            merge_state['manually_added_vulnerabilities'] = []
...
@public_ip_vapt_bp.route('/add_public_ip_vapt_vulnerability_details', methods=['POST'])
def add_public_ip_vulnerability_details():
    ...
    try:
        ...
        vuln_details_with_actual['actualVulnerabilityName'] = vuln_name
...
def update_public_ip_catalog_with_vulnerabilities(vulnerability_details_dict):
    ...
    try:
        ...
        print(f"🔍 update_public_ip_catalog_with_vulnerabilities called with {len(vulnerability_details_dict)} entries")
        ...
        try:
            ...
            print(f"✅ Read catalog with {len(catalog_df)} existing rows")
        except Exception as e:
        ...
        print(f"🔄 Processing {len(vulnerability_details_dict)} vulnerabilities")
        for vuln_name, details in vulnerability_details_dict.items():
            print(f"   Processing: {vuln_name}, isMerged: {details.get('isMerged', False)}, isManual: {details.get('isManual', False)}")
            ...
            if details.get('isMerged', False):
                ...
                print(f"📦 Processing non-merged vulnerability: {vuln_name}")
                ...
                new_row = {
                    ...
                    'Impact': details.get('impact', ''),
                    'Recommendation/Countermeasure': details.get('recommendation', ''),
                    'Affected System': details.get('affectedSystems', ''),  # Include affected systems
                    'Reference Link': details.get('referenceLink', ''),
                ...
                print(f"📝 Adding vulnerability to catalog: {details.get('vulnerabilityName', vuln_name)}")
                print(f"   New row data: {new_row}")
                
                # Add the new row to the catalog
                try:
                    catalog_df = pd.concat([catalog_df, pd.DataFrame([new_row])], ignore_index=True)
                    print(f"✅ Catalog DataFrame now has {len(catalog_df)} rows")
                except Exception as e:
                    print(f"❌ Error adding row to DataFrame: {e}")
                    import traceback
                    traceback.print_exc()
                    raise
        
        # Save the updated catalog to Sheet2
        print(f"\n💾 Saving catalog to file: {catalog_path}")
        ...
        try:
            ...
            print(f"✅ Successfully saved Public IP VAPT Catalog with {len(catalog_df)} total rows")
...
def create_public_ip_nmap_worksheet(wb, nmap_data_list):
    ...
    try:
        ...
        # Final pass: Reapply borders to all cells (merged cells may lose borders)
        for row_idx in range(1, current_row):
            for col_idx in range(1, 7):  # Columns A to F
                try:
                    ...
                    # Skip if cell is part of a merged range that can't be accessed directly
                    pass
        
        print(f"✅ Nmap Files worksheet created with {current_row - 1} rows")
...
def create_public_ip_summary_worksheet(wb, nessus_dataframes):
    ...
    try:
        ...
        if not nessus_dataframes:
            ...
            note_cell = ws.cell(row=3, column=1, value="Note: No vulnerabilities were identified during the Public IP VAPT Audit.")
...
def create_public_ip_Public_IP_VAPT_worksheet(wb, nessus_dataframes):
    """
    Create Public_IP_VAPT worksheet with data from Summary's Name column and match with catalog (using merge state).
    Similar to Infra_VAPT worksheet but adapted for Public IP VAPT data (no Branch Name, just Host).
    ...
    try:
        ...
        print("✅ Public_IP_VAPT worksheet created successfully (will populate with data if available)")
        ...
        merge_state = session.get('public_ip_vapt_vulnerability_merge_state', None)
        ...
        if not nessus_dataframes and not manually_added_vulnerabilities:
            print("⚠️ No Nessus data or manual vulnerabilities for Public_IP_VAPT worksheet - worksheet created with headers only")
            ...
            ws.row_dimensions[3].height = 30
            print("✅ Public_IP_VAPT worksheet created with headers and no data message")
            return
        ...
        if nessus_dataframes:
            ...
            if missing_cols:
                ...
                ws.row_dimensions[3].height = 30
                print("✅ Public_IP_VAPT worksheet created with headers only")
                return
...
def create_public_ip_metadata_worksheet(wb, form_data):
    """
...
@public_ip_vapt_bp.route('/process_public_ip_vapt_first_audit_report', methods=['POST'])
def process_public_ip_first_audit_report():
    """Main endpoint to process Public IP VAPT First Audit Report."""
    try:
        print("\n" + "="*80)
        print("🚀 Processing Public IP VAPT First Audit Report")
        print("="*80)
        ...
        nmap_data = process_nmap_zip_public_ip(nmap_file)  # Returns list of 6-element rows [HOST, PORT, SERVICE, HOST, PORT, SERVICE]
        ...
        create_public_ip_metadata_worksheet(wb, form_data)
        ...
        # Generate filename
        filename = generate_public_ip_filename(form_data['organization'], form_data['endDate'])
        ...
        try:
            ...
            for old_folder in old_folders:
                if os.path.isdir(old_folder) and old_folder != temp_poc_images_folder:
                    try:

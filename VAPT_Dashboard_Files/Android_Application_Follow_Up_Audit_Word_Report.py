...
android_follow_up_word_report_bp = Blueprint('android_app_follow_up_word_report', __name__)
...
def read_scope_worksheet_data(excel_file):
    ...
    try:
        ...
        print("⚠️ Could not import Android Mobile Application scope reader, falling back to empty list.")
        return []


def replace_followup_vulnerability_details_with_images(doc, excel_file):
    """
    Build the vulnerability section for the Android Mobile Application follow-up report.
    This mirrors the follow-up logic but reads from the 'Android Mobile Application VAPT' sheet.
    """
    try:
...
def extract_followup_poc_images_from_excel(excel_file):
    """
    Extract POC images from the Android Mobile Application follow-up Excel file.
    ...
    try:
        ...
        for sheet_name in wb.sheetnames:
            if "android mobile application vapt" in sheet_name.lower() or "android mobile application" in sheet_name.lower():
                ws = wb[sheet_name]
                break
        
        if not ws:
            print("❌ 'Android Mobile Application VAPT' worksheet not found")
            wb.close()
            return {}, {}, pd.DataFrame()
        ...
        if hasattr(ws, "_images"):
            print(f"📸 Found {len(ws._images)} images in Android Mobile Application sheet")
        ...
        # Print summary
        print(f"📊 Android extracted {len(poc_images_final)} vulnerabilities with POC (New) images")
        print(f"📊 Android extracted {len(old_poc_images)} vulnerabilities with Old POC images")
        print(f"📊 Android extracted {len(remarks_dict)} vulnerabilities with remarks")

        return poc_images_final, old_poc_images, df, remarks_dict

    except Exception as e:
        print(f"❌ Error extracting Android follow-up images: {e}")
        traceback.print_exc()
        return {}, {}, pd.DataFrame(), {}
...
def add_followup_images_at_end_of_document(doc, poc_images, old_poc_images, df):
    """
    Append old and new POC images for Android Mobile Application follow-up audit at the end of the document.
    """
    try:
        ...
        if not has_poc and not has_old_poc:
            print("⚠️ No Android POC images to append")
            return
        ...
        print(f"✅ Added {image_count} vulnerability sections with Android POC images")

    except Exception as e:
        print(f"❌ Error adding Android POC images: {e}")
        traceback.print_exc()
...
def replace_followup_poc_placeholders_with_images(doc, excel_file):
    ...
    try:
        ...
        print(f"❌ Error replacing Android follow-up POC placeholders: {e}")
        traceback.print_exc()
...
def replace_open_ports_with_nmap_table(doc, excel_file, placeholder="OPEN_PORTSSSS"):
    ...
    if not nmap_table:
        print("❌ Failed to create Android Nmap table")
        return False
    ...
    for paragraph in doc.paragraphs:
        if placeholder in paragraph.text:
            ...
            print(f"✅ Replaced '{placeholder}' with Android Nmap table")
            return True
            ...
            for cell in row.cells:
                for paragraph in cell.paragraphs:
                    if placeholder in paragraph.text:
                        ...
                        print(f"✅ Replaced '{placeholder}' with Android Nmap table inside table cell")
                        return True

    print(f"⚠️ Placeholder '{placeholder}' not found in Android document")
    return False
...
def replace_a11_row_with_vulnerabilities(doc, excel_file):
    ...
    try:
        ...
        df = pd.read_excel(excel_file, sheet_name='Android Mobile Application VAPT')

        if df.empty:
            print("No data found in 'Android Mobile Application VAPT' worksheet")
            return
...
def get_followup_vulnerability_counts(excel_file):
    """
    Extract vulnerability counts from "Android Mobile Application VAPT" worksheet:
...
def generate_follow_up_audit_word_report(excel_file_path, form_metadata=None):
    """
    Generate Follow-Up Audit Word report for Android Mobile Application from Excel file
    form_metadata: Dictionary containing metadata from form (if provided, will be used instead of Excel Meta_Data)
    """
    try:
        template_path = os.path.join("static", "Formats_and_Catalog", "Android_Mobile_Application_VAPT_Follow_Up_Audit_Report.docx")
        ...
        temp_dir = tempfile.mkdtemp()
        temp_file_path = os.path.join(temp_dir, "Android_Mobile_Application_VAPT_Follow_Up_Audit_Report.docx")
...
@android_follow_up_word_report_bp.route('/process_android_app_vapt_follow_up_audit_word_report', methods=['POST'])
def process_android_app_vapt_follow_up_audit_word_report():
    """
    Process the uploaded Excel file and generate an Android Mobile Application Follow-Up Audit Word report
    """
    try:
        ...
        for idx in range(len(asset_descriptions)):
            if asset_descriptions[idx] and asset_criticalities[idx] and asset_hashes[idx] and asset_versions[idx]:
                asset_info = {
        ...
        try:
            ...
            return send_file(
                word_file_path,
                as_attachment=True,
                download_name='Android_Mobile_Application_VAPT_Follow_Up_Audit_Report.docx',
        ...
        print(f"❌ Error processing android application follow-up audit word report: {e}")
        traceback.print_exc()

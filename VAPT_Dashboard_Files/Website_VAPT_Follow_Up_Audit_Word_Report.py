...
website_vapt_follow_up_word_report_bp = Blueprint('website_vapt_follow_up_word_report', __name__)
...
def replace_followup_vulnerability_details_with_images(doc, excel_file):
    ...
    try:
        ...
        for idx, row in enumerate(df.iterrows(), start=1):
            ...
            # Get data from column index 9 (same as line 2362)
...
def create_nmap_table_from_excel(doc, excel_file):
    ...
    try:
        ...
        # For Website VAPT, only use columns A-C
        ...
        for merged_range in ws.merged_cells:
            ...
            # Skip if merged cell is outside columns A-C
            if start_col >= 3:
                continue
            
            bottom_right_cell_ref = merged_range_coords[-1]
            end_row = bottom_right_cell_ref[0] - 1
            end_col = min(bottom_right_cell_ref[1] - 1, 2)  # Limit to column C
        ...
        for i, row in enumerate(ws.iter_rows(values_only=False)):
            # Only process first 3 columns (A-C)
...
def replace_a11_row_with_vulnerabilities(doc, excel_file):
    ...
    try:
        ...
        if df.empty:
            print("No data found in 'Website_VAPT' worksheet")
            return
        ...
        for idx in range(len(df)):
            ...
            # Column 2: Affected System (Excel column K, index 10) - Affected Systems
            second_cell = new_row.cells[1]
            for paragraph in second_cell.paragraphs:
                paragraph.clear()
            affected_system = str(df.iloc[idx, 9]) if pd.notna(df.iloc[idx, 10]) else "NA"
...
def generate_follow_up_audit_word_report(excel_file_path):
    ...
    try:
        ...
        template_path = os.path.join("static", "Formats_and_Catalog", "Website_VAPT_Follow_Up_Audit_Report.docx")
        ...
        temp_dir = tempfile.mkdtemp()
        temp_file_path = os.path.join(temp_dir, "Website_VAPT_Follow_Up_Audit_Report.docx")
...
@website_vapt_follow_up_word_report_bp.route('/process_website_vapt_follow_up_audit_word_report', methods=['POST'])
def process_website_follow_up_audit_word_report():
    """
    Process the uploaded Excel file and generate a Follow-Up Audit Word report
    """
    try:
        # Check if file was uploaded
        if 'websiteFollowUpExcelFile' not in request.files:
            return jsonify({'error': 'No file uploaded'}), 400
        
        file = request.files['websiteFollowUpExcelFile']
        ...
        try:
            ...
            return send_file(
                word_file_path,
                as_attachment=True,
                download_name='Website_VAPT_Follow_Up_Audit_Report.docx',

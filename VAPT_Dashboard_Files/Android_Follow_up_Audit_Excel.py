# Android Mobile Application VAPT Follow-Up Audit Excel Report Generator
...
# Create a Blueprint for Android Mobile Application VAPT Follow-Up routes
android_app_vapt_followup_bp = Blueprint('android_app_vapt_followup', __name__)

def generate_android_app_followup_filename(organization, end_date):
    """
    Generate static filename for Android Mobile Application VAPT Follow-Up Audit.
    Format: Android_Application_VAPT_Follow_Up_Audit
    """
    return "Android_Application_VAPT_Follow_Up_Audit_Report.xlsx"
...
def find_latest_status_column_in_user_excel(user_excel_path, ws_name="Android Mobile Application VAPT"):
    ...
    try:
        ...
        for sheet_name in user_wb.sheetnames:
            if ws_name.lower() in sheet_name.lower() or "android mobile application" in sheet_name.lower():
...
def extract_poc_images_from_user_excel(user_excel_path, ws_name="Android Mobile Application VAPT"):
    ...
    try:
        ...
        temp_dir = f"Temp_POC_AndroidApp_{timestamp}"
        ...
        for section_title in sections_to_copy:
            ...
            if section_start_row:
                ...
                if section_upper == "ORGANIZATION INFORMATION":
                    ...
                    # IMPORTANT: Stop before Revalidation Cycles to avoid duplication
                    ...
                    for end_idx in range(section_start_row + 1, user_meta_ws.max_row + 1):
                        ...
                        if next_cell and str(next_cell).strip():
                            ...
                            # Stop if we encounter a Revalidation Cycle (will be copied separately)
                            if 'revalidation cycle' in cell_str.lower():
...
def find_next_revalidation_cycle_number(user_excel_path=None, target_ws=None):
    ...
    cycle_numbers = set()  # Use set to avoid duplicates
...
def create_android_app_followup_metadata_worksheet(wb, form_data, user_excel_path=None):
    ...
    try:
        ...
        # Write Hash Value if provided
        hash_value = form_data.get('hashValue', '').strip()
        if hash_value:
            cell = ws.cell(row=current_row, column=1, value='Hash Value')
            cell.font = data_font
            cell.alignment = data_alignment
            cell.border = thin_border
            
            cell = ws.cell(row=current_row, column=2, value=hash_value)
            cell.font = data_font
            cell.alignment = data_alignment
            cell.border = thin_border
...
@android_app_vapt_followup_bp.route('/process_android_app_vapt_follow_up_audit_report', methods=['POST'])
def process_android_app_follow_up_audit_report():
    """Main endpoint to process Android Mobile Application VAPT Follow-Up Audit Report."""
    temp_files = []
    temp_dirs = []
    
    try:
        print("\n" + "="*80)
        print("🚀 Processing Android Mobile Application VAPT Follow-Up Audit Report")
        print("="*80)
        ...
        form_data = {
            ...
            'hashValue': request.form.get('hashValue', '').strip(),
        }
        ...
        user_ws_name = None
        for sheet_name in user_wb.sheetnames:
            if 'android mobile application vapt' in sheet_name.lower():
        ...
        # Copy "Android Mobile Application VAPT" worksheet from user Excel
        copy_success, temp_image_files = copy_worksheet_with_images(user_wb, user_ws_name, wb, "Android Mobile Application VAPT")
        if not copy_success:
            user_wb.close()
        ...
        if user_status_col:
            try:
                ...
                user_ws = None
                for sheet_name in user_wb.sheetnames:
                    if user_ws_name.lower() in sheet_name.lower() or "android mobile application" in sheet_name.lower():
        ...
        create_android_app_followup_metadata_worksheet(wb, form_data, user_excel_path)
        ...
        filename = generate_android_app_followup_filename('', form_data.get('endDate', ''))

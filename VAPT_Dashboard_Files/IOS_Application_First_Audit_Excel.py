# IOS Mobile Application VAPT First Audit Excel Report Generator
...
# Create a Blueprint for IOS Mobile Application VAPT routes
ios_app_vapt_bp = Blueprint('ios_app_vapt', __name__)

def generate_ios_app_filename(organization, end_date):
    """
    Generate static filename for IOS Mobile Application VAPT First Audit.
    Format: IOS_Application_VAPT_First_Audit
    """
    return "IOS_Application_VAPT_First_Audit_Report.xlsx"
...
def insert_poc_images_to_excel(excel_path, poc_mapping, vulnerabilities_data):
    ...
    try:
        ...
        # Get the IOS Mobile Application VAPT worksheet
        if "IOS Mobile Application VAPT" not in wb.sheetnames:
...
def create_ios_app_metadata_worksheet(wb, form_data):
    """
...
def create_ios_app_vapt_worksheet(wb, vulnerabilities_data):
    """
    Create IOS Mobile Application VAPT worksheet with headers and vulnerability data.
    ...
    """
    try:
        print("📊 Creating IOS Mobile Application VAPT worksheet...")
        
        # Create worksheet
        ws = wb.create_sheet("IOS Mobile Application VAPT")
        ...
        print(f"✅ Created IOS Mobile Application VAPT worksheet with {len(vulnerabilities_data)} vulnerabilities")
        
    except Exception as e:
        print(f"❌ Error creating IOS Mobile Application VAPT worksheet: {e}")
        import traceback
        traceback.print_exc()

@ios_app_vapt_bp.route('/process_ios_app_vapt_first_audit_report', methods=['POST'])
def process_ios_app_first_audit_report():
    """Main endpoint to process IOS Mobile Application VAPT First Audit Report."""
    try:
        print("\n" + "="*80)
        print("🚀 Processing IOS Mobile Application VAPT First Audit Report")
        print("="*80)
        ...
        vulnerability_status = request.form.get('iosAppVulnerabilityStatus', 'has')
        ...
        # Create IOS Mobile Application VAPT worksheet
        create_ios_app_vapt_worksheet(wb, vulnerabilities_data)
        
        # Create Meta_Data worksheet
        create_ios_app_metadata_worksheet(wb, form_data)
        ...
        # Generate filename
        filename = generate_ios_app_filename(
        ...
        print(f"❌ Error processing IOS Mobile Application VAPT First Audit Report: {e}")
        import traceback
        traceback.print_exc()

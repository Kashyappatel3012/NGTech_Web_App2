# vapt_follow_up_audit_routes.py
...
# Create a Blueprint for VAPT follow up audit routes
vapt_follow_up_audit_bp = Blueprint('vapt_follow_up_audit', __name__)
        ...
        for vuln_name in closed_vulnerabilities:
            ...
            if vuln_name in user_row_data:
                ...
                for col_idx, header in enumerate(headers, 1):
                    ...
                    if cell_value is not None and pd.notna(cell_value):
                        # Handle different data types with Excel injection protection
                        ...
                        if isinstance(cell_value, (int, float)):
                            ...
                            cell_str = str(cell_value).strip()
                            if cell_str != "":
                                # Sanitize string values to prevent Excel injection
                                cell.value = sanitize_excel_value(cell_str)
        ...
        with zipfile.ZipFile(BytesIO(zip_data), 'r') as zip_ref:
            ...
            for file_name in nessus_files:
                try:
                    ...
                    device_types = ["Servers", "Server", "Systems", "System", "Switch", "Router", 'Firewall', 'Workstations', 'Access Points', 'CCTV']
...
@vapt_follow_up_audit_bp.route('/vapt_followup_check_vulnerabilities', methods=['POST'])
def followup_check_vulnerabilities():
    ...
    try:
        ...
        if nessus_dataframes:
            ...
            session['vapt_followup_vulnerability_merge_state'] = {
...
@vapt_follow_up_audit_bp.route('/vapt_followup_submit_vulnerability_details', methods=['POST'])
def submit_vulnerability_details():
    ...
    try:
        ...
        session['vapt_followup_unmatched_vulnerability_details'] = vulnerability_details
...
@vapt_follow_up_audit_bp.route('/vapt_followup_merge_with_matched', methods=['POST'])
def followup_merge_with_matched():
    ...
    try:
        ...
        merge_state = session.get('vapt_followup_vulnerability_merge_state', {})
        if not merge_state:
        ...
        # Update session
        session['vapt_followup_vulnerability_merge_state'] = merge_state
...
@vapt_follow_up_audit_bp.route('/vapt_followup_merge_with_unmatched', methods=['POST'])
...
@vapt_follow_up_audit_bp.route('/vapt_followup_add_vulnerability_details', methods=['POST'])
...
@vapt_follow_up_audit_bp.route('/vapt_followup_merge_matched_groups', methods=['POST'])
...
@vapt_follow_up_audit_bp.route('/vapt_followup_undo_last_merge', methods=['POST'])
...
@vapt_follow_up_audit_bp.route('/process_vapt_follow_up_audit_report', methods=['POST'])
    ...
    try:
        ...
        with pd.ExcelWriter(output, engine='xlsxwriter', engine_kwargs={'options': {'nan_inf_to_errors': True}}) as writer:
            ...
            if nessus_dataframes:
                ...
                unmatched_details_mapping = session.get('vapt_followup_unmatched_vulnerability_details', {})
        ...
        try:
            ...
            if 'Infra_VAPT' in wb.sheetnames:
                ...
                # Step 4: Add "N/A" to empty cells in column E (CVE/CWE ID) for rows where column A has content
                try:
                    from openpyxl.styles import Alignment
                    ws_infra = wb['Infra_VAPT']
                    max_row = ws_infra.max_row
                    column_a = 1  # Column A (Sr.No)
                    column_e = 5  # Column E (CVE/CWE ID)
                    na_count = 0
                    ...
                    if na_count > 0:
                        print(f"✅ Added 'N/A' to {na_count} empty cells in column E (CVE/CWE ID) for rows where column A has content")
                    else:
                        print(f"✅ Checked column E (CVE/CWE ID) - all cells have content for rows where column A has content")
                except Exception as na_err:
                    print(f"⚠️ Could not add N/A to empty CVE/CWE ID cells: {na_err}")
                    import traceback
                    traceback.print_exc()
                
                # Step 5: Save all changes at once
                wb.save(temp_final_path)
                print("✅ Saved workbook with all updates (row heights + Meta_Data counts + column formatting + CVE/CWE ID N/A)")
...
@vapt_follow_up_audit_bp.route('/vapt_extract_images_from_excel', methods=['POST'])
...
@vapt_follow_up_audit_bp.route('/vapt_get_images_info_from_excel', methods=['POST'])
...
def create_infra_vapt_worksheet(workbook, combined_nessus, header_format, cell_format, unmatched_details_mapping=None, evidence_files=None):
    ...
    try:
        ...
        merge_state = session.get('vapt_followup_vulnerability_merge_state', None)
            ...
            for catalog_idx in sorted_catalog_indices:
                ...
                for i in range(insert_position):
                    ...
                    if risk_factor_col is not None and i == risk_factor_col:
                        ...
                        # Special handling for CVE/CWE ID column - fill with "N/A" if empty
...
@vapt_follow_up_audit_bp.route('/vapt_add_old_poc_images', methods=['POST'])

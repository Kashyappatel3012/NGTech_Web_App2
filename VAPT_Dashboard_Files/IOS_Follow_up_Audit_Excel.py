# IOS Mobile Application VAPT Follow-Up Audit Excel Report Generator
...
# Create a Blueprint for IOS Mobile Application VAPT Follow-Up routes
ios_app_vapt_followup_bp = Blueprint('ios_app_vapt_followup', __name__)

def generate_ios_app_followup_filename(organization, end_date):
    """
    Generate static filename for IOS Mobile Application VAPT Follow-Up Audit.
    Format: IOS_Application_VAPT_Follow_Up_Audit
    """
    return "IOS_Application_VAPT_Follow_Up_Audit_Report.xlsx"
...
def find_latest_status_column_in_user_excel(user_excel_path, ws_name="IOS Mobile Application VAPT"):
    ...
    try:
        ...
        for sheet_name in user_wb.sheetnames:
            if ws_name.lower() in sheet_name.lower() or "ios mobile application" in sheet_name.lower() or "ios application" in sheet_name.lower():
...
def extract_poc_images_from_user_excel(user_excel_path, ws_name="IOS Mobile Application VAPT"):
    ...
    try:
        ...
        temp_dir = f"Temp_POC_IOSApp_{timestamp}"
...
def create_ios_app_followup_metadata_worksheet(wb, form_data, user_excel_path=None):
...
@ios_app_vapt_followup_bp.route('/process_ios_app_vapt_follow_up_audit_report', methods=['POST'])
def process_ios_app_vapt_follow_up_audit_report():
    """Main endpoint to process IOS Mobile Application VAPT Follow-Up Audit Report."""
    temp_files = []
    temp_dirs = []
    
    try:
        print("\n" + "="*80)
        print("🚀 Processing IOS Mobile Application VAPT Follow-Up Audit Report")
        print("="*80)
        ...
        user_ws_name = None
        for sheet_name in user_wb.sheetnames:
            if 'ios mobile application vapt' in sheet_name.lower() or 'ios application vapt' in sheet_name.lower():
        ...
        # Copy "IOS Mobile Application VAPT" worksheet from user Excel
        copy_success, temp_image_files = copy_worksheet_with_images(user_wb, user_ws_name, wb, "IOS Mobile Application VAPT")
        if not copy_success:
            user_wb.close()
        ...
        if user_status_col:
            try:
                ...
                user_ws = None
                for sheet_name in user_wb.sheetnames:
                    if user_ws_name.lower() in sheet_name.lower() or "ios mobile application" in sheet_name.lower() or "ios application" in sheet_name.lower():
        ...
        create_ios_app_followup_metadata_worksheet(wb, form_data, user_excel_path)
        ...
        filename = generate_ios_app_followup_filename('', form_data.get('endDate', ''))

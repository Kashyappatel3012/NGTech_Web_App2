...
# Create blueprint for Public IP First Audit MetaData
public_ip_first_audit_metadata_bp = Blueprint('public_ip_first_audit_metadata', __name__)

def create_public_ip_first_audit_metadata_excel(form_data):
    """Create Public IP First Audit MetaData Excel file"""
    try:
        ...
        if vulnerability_status == '0':
            ...
            note_text = "Note: No vulnerabilities were identified during the first Infrastructure VAPT audit."
        ...
        filename = 'Public_IP_First_Audit_MetaData.xlsx'
        ...
        if os.path.exists(filepath):
            print(f"🔍 DEBUG: Removing existing file to avoid overwrite issues...")
            os.remove(filepath)
...
@public_ip_first_audit_metadata_bp.route('/process_public_ip_first_audit_metadata', methods=['POST'])
def process_public_ip_first_audit_metadata():
    """Process Public IP First Audit MetaData form submission"""
    try:
        print("\n" + "="*80)
        print("🚀 Processing Public IP First Audit MetaData")
        print("="*80)
        ...
        # Create Excel file
        filepath, filename = create_public_ip_first_audit_metadata_excel(form_data)
        print(f"\n✅ Excel file created: {filename}")
        print(f"🔍 DEBUG: File path: {filepath}")
        print("="*80)
        
        # Return file info as JSON with download endpoint
        return jsonify({
            'success': True,
            'filename': filename,
            'download_url': f'/download_public_ip_first_audit_metadata/{filename}'
        })
        ...
        logger.error(f"Error processing public IP first audit metadata: {type(e).__name__}: {str(e)}", exc_info=True)
        # Return safe error message to client
        return jsonify({'success': False, 'error': 'An error occurred processing the request. Please try again later.'}), 500

# Download endpoint that deletes file after sending
@public_ip_first_audit_metadata_bp.route('/download_public_ip_first_audit_metadata/<filename>', methods=['GET'])
def download_public_ip_first_audit_metadata(filename):
    """Download Public IP First Audit MetaData Excel file and delete it after download"""
    try:
        filepath = os.path.join('static', 'uploads', filename)
        ...
        try:
            os.remove(filepath)
            print(f"✅ Deleted file after reading: {filename}")
        except Exception as e:
        ...
        import logging
        logger = logging.getLogger(__name__)
        logger.error(f"Error downloading public IP first audit metadata file: {type(e).__name__}: {str(e)}", exc_info=True)
        # Return safe error message to client
        return jsonify({'success': False, 'error': 'An error occurred downloading the file. Please try again later.'}), 500

# Cleanup endpoint
@public_ip_first_audit_metadata_bp.route('/cleanup_public_ip_first_audit_metadata', methods=['POST'])
def cleanup_public_ip_first_audit_metadata():
    """Clean up old Public IP First Audit MetaData Excel files"""
    try:
        uploads_dir = os.path.join('static', 'uploads')
        if os.path.exists(uploads_dir):
            for filename in os.listdir(uploads_dir):
                if filename.startswith('Public_IP_First_Audit_MetaData') and filename.endswith('.xlsx'):
        ...
        import logging
        logger = logging.getLogger(__name__)
        logger.error(f"Error cleaning up public IP first audit metadata: {type(e).__name__}: {str(e)}", exc_info=True)
        # Return safe error message to client

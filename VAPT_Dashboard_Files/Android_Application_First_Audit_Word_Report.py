...
android_app_word_report_bp = Blueprint('android_app_word_report', __name__)


def _extract_android_asset_hash_values(excel_file_path):
...
def _prepare_android_excel_for_web_logic(original_excel_path):
    """
    Create a temporary copy of the Android Excel file with worksheet names tailored for
    ...
    try:
        ...
        if 'Android Mobile Application VAPT' in wb.sheetnames and 'Web Application VAPT' not in wb.sheetnames:
            ws = wb['Android Mobile Application VAPT']
            ws.title = 'Web Application VAPT'

        wb.save(temp_file.name)
        wb.close()
    except Exception as e:
        print(f"⚠️ Error preparing Android Excel for Word logic: {e}")
        traceback.print_exc()
...
def _check_empty_a2_and_handle(doc, original_excel_file, worksheet_name):
    """Check if A2 is empty in the specified worksheet and handle accordingly"""
    try:
...
def _check_empty_a2_and_handle_android(doc, original_excel_file):
    """Check if A2 is empty in Android Mobile Application VAPT worksheet and handle accordingly"""
    try:
        wb = load_workbook(original_excel_file, data_only=True)
        if 'Android Mobile Application VAPT' in wb.sheetnames:
            ws = wb['Android Mobile Application VAPT']
            a2_value = ws.cell(row=2, column=1).value  # A2 is row 2, column 1 (1-indexed)
            # Check if A2 is empty or None
            if a2_value is None or str(a2_value).strip() == '':
...
def _check_empty_a2_for_a11_android(doc, original_excel_file):
    """Check if A2 is empty in Android Mobile Application VAPT worksheet for a11 function"""
    try:
        df = pd.read_excel(original_excel_file, sheet_name='Android Mobile Application VAPT')
...
def generate_android_word_report_from_excel(excel_file_path):
    """
    Generate Android Mobile Application First Audit Word report by leveraging the
    Web Application Word report logic and applying Android-specific placeholder replacements.
    """
    temp_excel_for_web = _prepare_android_excel_for_web_logic(excel_file_path)

    try:
        template_path = os.path.join('static', 'Formats_and_Catalog', 'Android_Mobile_Application_VAPT_First_Audit_Report.docx')
        ...
        if _check_empty_a2_and_handle_android(doc, excel_file_path):
        ...
        if not _check_empty_a2_for_a11_android(doc, excel_file_path):
        ...
        hash_values = _extract_android_asset_hash_values(excel_file_path)
        _replace_hash_placeholders_in_doc(doc, hash_values)

        print("✅ Android Mobile Application Word report generated successfully from Excel file")
        return doc
...
@android_app_word_report_bp.route('/process_android_app_vapt_first_audit_word_report', methods=['POST'])
def process_android_app_first_audit_word_report():
    """Process the uploaded Excel file and generate an Android Mobile Application Word report."""
    try:
        ...
        try:
            print("🔍 Starting Android Mobile Application Word report generation...")
            doc = generate_android_word_report_from_excel(temp_excel_path)
            ...
            response = send_file(
                temp_doc_path,
                as_attachment=True,
                download_name='Android_Mobile_Application_VAPT_First_Audit_Report.docx',
            ...
            print("✅ Android Mobile Application Word report generated successfully.")
            return response

        except Exception as processing_error:
            print(f"❌ Error generating Android Word report: {processing_error}")
            traceback.print_exc()
        ...
        print(f"❌ Error in Android Word report route: {e}")
        traceback.print_exc()

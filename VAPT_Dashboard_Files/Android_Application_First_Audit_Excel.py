# Android Mobile Application VAPT First Audit Excel Report Generator
...
# Create a Blueprint for Android Mobile Application VAPT routes
android_app_vapt_bp = Blueprint('android_app_vapt', __name__)

def generate_android_app_filename(organization, end_date):
    """
    Generate static filename for Android Mobile Application VAPT First Audit.
    Format: Android_Application_VAPT_First_Audit
    """
    return "Android_Application_VAPT_First_Audit_Report.xlsx"
...
def insert_poc_images_to_excel(excel_path, poc_mapping, vulnerabilities_data):
    ...
    try:
        ...
        # Get the Android Mobile Application VAPT worksheet
        if "Android Mobile Application VAPT" not in wb.sheetnames:
...
def create_android_app_metadata_worksheet(wb, form_data):
    """
        ...
        if assets:
            for i, asset in enumerate(assets):
                asset_data = [
                    ...
                    (f'Asset {i+1} - Hash Value', asset.get('hash_value', '')),
...
def create_android_app_vapt_worksheet(wb, vulnerabilities_data):
    """
    Create Android Mobile Application VAPT worksheet with headers and vulnerability data.
    ...
    """
    try:
        print("📊 Creating Android Mobile Application VAPT worksheet...")
        
        # Create worksheet
        ws = wb.create_sheet("Android Mobile Application VAPT")
        ...
        ws.column_dimensions['K'].width = 30  # Affected Systems
        ...
        print(f"✅ Created Android Mobile Application VAPT worksheet with {len(vulnerabilities_data)} vulnerabilities")
        
    except Exception as e:
        print(f"❌ Error creating Android Mobile Application VAPT worksheet: {e}")
        import traceback
        traceback.print_exc()

@android_app_vapt_bp.route('/process_android_app_vapt_first_audit_report', methods=['POST'])
def process_android_app_first_audit_report():
    """Main endpoint to process Android Mobile Application VAPT First Audit Report."""
    try:
        print("\n" + "="*80)
        print("🚀 Processing Android Mobile Application VAPT First Audit Report")
        print("="*80)
        ...
        asset_hash_values = request.form.getlist('assetHash[]')
        ...
        for i in range(len(asset_descriptions)):
            ...
            criticality = asset_criticalities[i].strip() if i < len(asset_criticalities) and asset_criticalities[i] else ''
            hash_value = asset_hash_values[i].strip() if i < len(asset_hash_values) and asset_hash_values[i] else ''
        ...
        vulnerability_status = request.form.get('androidAppVulnerabilityStatus', 'has')
        ...
        # Create Android Mobile Application VAPT worksheet
        create_android_app_vapt_worksheet(wb, vulnerabilities_data)
        
        # Create Meta_Data worksheet
        create_android_app_metadata_worksheet(wb, form_data)
        ...
        # Generate filename
        filename = generate_android_app_filename(
        ...
        print(f"❌ Error processing Android Mobile Application VAPT First Audit Report: {e}")
        import traceback
        traceback.print_exc()

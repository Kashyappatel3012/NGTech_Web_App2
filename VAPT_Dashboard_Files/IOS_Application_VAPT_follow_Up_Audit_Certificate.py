...
# Create blueprint for IOS Application VAPT Follow Up Audit Certificate generation
ios_app_vapt_follow_up_audit_certificate_bp = Blueprint('ios_app_vapt_follow_up_audit_certificate', __name__)
...
def generate_ios_app_vapt_follow_up_audit_certificate(organization_name, city, state, auditee_designation, scope,
                                                      start_audit_date, end_audit_date, first_report_id, report_id):
    """Generate an IOS Application VAPT Follow Up Audit Completion Certificate document."""
    try:
        template_path = os.path.join('static', 'Formats_and_Catalog', 'IOS_Application_VAPT_Follow_Up_Audit_Conduct_Certificate.docx')
        ...
        import shutil
        certificate_filename = "IOS_Application_VAPT_Follow_Up_Certificate.docx"
        certificate_path = os.path.join(temp_dir, certificate_filename)
        shutil.copy2(template_path, certificate_path)

        print(f"✅ IOS Application certificate template copied successfully: {certificate_path}")
        ...
        replacements = {
            ...
            "XXXXXXXXX": first_report_id
        }

        print("🔄 Replacing placeholders in IOS Application certificate...")
        replace_text_in_document(doc, replacements)

        doc.save(certificate_path)

        print(f"✅ IOS Application certificate generated successfully with replaced data: {certificate_path}")
        print("📋 IOS Application Certificate Data Applied:")
        ...
        print(f"   First Report ID: {first_report_id}")
        print(f"   Follow-up Report ID: {report_id}")

        return certificate_path

    except Exception as e:
        print(f"❌ Error generating IOS Application certificate: {e}")
        traceback.print_exc()
        raise e


@ios_app_vapt_follow_up_audit_certificate_bp.route('/process_ios_app_vapt_follow_up_audit_certificate', methods=['POST'])
def process_ios_app_vapt_follow_up_audit_certificate():
    """Process the form data and generate an IOS Application VAPT Follow Up Audit Completion Certificate."""
    try:
        ...
        first_report_id = request.form.get('firstReportId', '').strip()
        ...
        if not all([organization_name, city, state, auditee_designation, scope, start_audit_date, end_audit_date, first_report_id, report_id]):
            return jsonify({'error': 'All fields are required'}), 400

        print("📋 IOS Application Follow Up Certificate Data Received:")
        print(f"   Organization: {organization_name}")
        print(f"   City: {city}")
        print(f"   State: {state}")
        print(f"   Auditee Designation: {auditee_designation}")
        ...
        certificate_path = generate_ios_app_vapt_follow_up_audit_certificate(
        ...
        return send_file(
            certificate_path,
            as_attachment=True,
            download_name="IOS_Application_VAPT_Follow_Up_Certificate.docx",
            mimetype='application/vnd.openxmlformats-officedocument.wordprocessingml.document'
        )

    except Exception as e:
        print(f"❌ Error processing IOS Application follow up audit certificate: {e}")
        traceback.print_exc()

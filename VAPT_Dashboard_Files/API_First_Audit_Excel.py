# API VAPT First Audit Excel Report Generator
...
# Create a Blueprint for API VAPT routes
api_vapt_bp = Blueprint('api_vapt', __name__)

def generate_api_filename(organization, end_date):
    """
    Generate static filename for API VAPT First Audit.
    Format: API_VAPT_First_Audit
    """
    return "API_VAPT_First_Audit_Report.xlsx"
...
def insert_poc_images_to_excel(excel_path, poc_mapping, vulnerabilities_data):
    ...
    try:
        ...
        # Get the API VAPT worksheet
        if "API VAPT" not in wb.sheetnames:
...
def create_api_metadata_worksheet(wb, form_data):
    """
    ...
    try:
        ...
        metadata_sections = [
            ...
            },
            {
                ...
                'data': [
                    ('Application Type', form_data.get('applicationType', ''))
                ]
            },
            {
...
def create_api_vapt_worksheet(wb, vulnerabilities_data):
    """
    Create API VAPT worksheet with headers and vulnerability data.
    ...
    """
    try:
        print("📊 Creating API VAPT worksheet...")
        
        # Create worksheet
        ws = wb.create_sheet("API VAPT")
        ...
        print(f"✅ Created API VAPT worksheet with {len(vulnerabilities_data)} vulnerabilities")
        
    except Exception as e:
        print(f"❌ Error creating API VAPT worksheet: {e}")
        import traceback
        traceback.print_exc()

@api_vapt_bp.route('/process_api_vapt_first_audit_report', methods=['POST'])
def process_api_first_audit_report():
    """Main endpoint to process API VAPT First Audit Report."""
    try:
        print("\n" + "="*80)
        print("🚀 Processing API VAPT First Audit Report")
        print("="*80)
        ...
        vulnerability_status = request.form.get('apiVulnerabilityStatus', 'has')
        ...
        # Create API VAPT worksheet
        create_api_vapt_worksheet(wb, vulnerabilities_data)
        
        # Create Meta_Data worksheet
        create_api_metadata_worksheet(wb, form_data)
        ...
        # Generate filename
        filename = generate_api_filename(
        ...
        print(f"❌ Error processing API VAPT First Audit Report: {e}")
        import traceback
        traceback.print_exc()

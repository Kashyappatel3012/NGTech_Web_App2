...
website_vapt_word_report_bp = Blueprint('website_vapt_word_report', __name__)
        ...
        for row in sheet.iter_rows():
            for cell in row:
                if cell.value is not None:
                    ...
                    # Special handling for "ORGANIZATION EMAIL ADDRESSES"
                    if str(cell.value).strip() == "ORGANIZATION EMAIL ADDRESSES":
                        # Start from the cell below and to the right (next column, next row)
                        current_row = cell.row + 1
                        current_col = cell.column + 1
...
def create_nmap_table_from_excel(doc, excel_file):
    ...
    try:
        ...
        # Create a table with only 3 columns (HOST, PORT, SERVICE)
        # But use Excel max_row to keep all data rows
        table = doc.add_table(rows=ws.max_row, cols=3)
        table.style = 'Table Grid'

        # A set to keep track of merged cells to avoid overwriting them
        merged_cells_to_skip = set()

        # First, handle merged cells (only for columns A-C)
        for merged_range in ws.merged_cells:
            ...
            # Only process merged cells that are within first 3 columns (A-C)
            if end_col >= 3:
                continue
        ...
        # Fill Word table with Excel data and formatting (only first 3 columns)
        for i, row in enumerate(ws.iter_rows(values_only=False)):
            for j, cell in enumerate(row):
                # Only process first 3 columns (A, B, C)
                if j >= 3:
                    break
                ...
                word_cell.text = ""
                # Add run with value
                run = word_cell.paragraphs[0].add_run(value)
                
                # Always set Calibri font for all cells
                run.font.name = "Calibri"
                run.font.size = Pt(12)
                
                # Apply formatting from Excel cell if available
                if cell.font:
...
def extract_poc_images_from_excel(excel_file):
    ...
    try:
        ...
        print("Worksheet 'Website_VAPT' not found in Excel file. Returning empty data.")
        return {}, pd.DataFrame()
    except Exception as e:
...
def replace_metadata_placeholders(doc, metadata):
    ...
    try:
        ...
        replacements = {
            ...
            "Auditee_Email_Adresss": bank_emails,  # Use bank emails from metadata
            "Monthh Yearr": month_year,
            "00rd Month Year to 00th Month Year": date_range
        }
        
        print(f"🔍 Bank emails to replace 'Auditee_Email_Adresss': {bank_emails}")
        ...
        for paragraph in doc.paragraphs:
            for old_text, new_text in replacements.items():
                if old_text in paragraph.text:
                    found_placeholders.append(old_text)
        ...
        print(f"📝 Found placeholders in paragraphs: {found_placeholders}")
            ...
            for row in table.rows:
                for cell in row.cells:
                    for paragraph in cell.paragraphs:
                        for old_text, new_text in replacements.items():
                            if old_text in paragraph.text:
                                found_in_tables.append(old_text)
                                # Special handling for date_range to make it bold
                                if old_text == "00rd Month Year to 00th Month Year":
                                    _replace_text_in_runs_with_bold(paragraph, old_text, new_text, make_bold=True)
                                else:
        ...
        print(f"📝 Found placeholders in tables: {found_in_tables}")
...
def replace_affected_systems_with_table(table, placeholder, affected_systems_text):
    """
    Replace the placeholder with an actual Word table containing affected systems/URLs data.
    This function handles URLs, IPs, or any text data and displays it in a tabular format.
    """
    target_cell = None
    for row in table.rows:
        for cell in row.cells:
    ...
    if not affected_systems_text or affected_systems_text.strip() == '' or affected_systems_text.strip() == 'NA' or affected_systems_text.strip() == 'nan':
        target_cell.text = "No affected systems"
        return
    
    # Split the text by newlines or other separators to create table rows
    # Handle multiple separators: newline, comma, semicolon, or pipe
    lines = []
    if '\n' in affected_systems_text:
        lines = [line.strip() for line in affected_systems_text.strip().split('\n') if line.strip()]
    elif ',' in affected_systems_text:
        lines = [line.strip() for line in affected_systems_text.strip().split(',') if line.strip()]
    elif ';' in affected_systems_text:
        lines = [line.strip() for line in affected_systems_text.strip().split(';') if line.strip()]
    elif '|' in affected_systems_text:
        lines = [line.strip() for line in affected_systems_text.strip().split('|') if line.strip()]
        ...
        # Single line/item
        lines = [affected_systems_text.strip()]
    
    # Filter out empty lines
    lines = [line for line in lines if line and line.strip() != '' and line.strip().lower() != 'na' and line.strip().lower() != 'nan']
    ...
    # Create single-column table with all the data
    new_table = target_cell.add_table(rows=len(lines), cols=1)
    
    new_table.alignment = WD_TABLE_ALIGNMENT.LEFT
    
    # Populate table with data and apply formatting
    for i, row in enumerate(new_table.rows):
        row.cells[0].width = Inches(3.5)  # Single column width
        ...
        # Apply font formatting to paragraphs in cell
        for paragraph in cell.paragraphs:
...
def replace_vulnerability_details_with_images(doc, excel_file):
    ...
    """
    try:
        # Check if A2 cell is empty in Website_VAPT worksheet
        try:
                ...
                if a2_value is None or str(a2_value).strip() == '':
                    ...
                    for paragraph in doc.paragraphs:
                        if "Tables_Heree" in paragraph.text:
                            ...
                            run = paragraph.add_run("Note: No vulnerabilities were identified during the Website VAPT Audit.")
                    ...
                    for paragraph in doc.paragraphs:
                        for idx, run in enumerate(paragraph.runs):
                            if "Tables_Heree" in run.text:
                                ...
                                run.text = "Note: No vulnerabilities were identified during the Website VAPT Audit."
                                run.font.color.rgb = RGBColor(0, 128, 0)  # Green color
                                run.font.bold = True
                                paragraph.alignment = WD_ALIGN_PARAGRAPH.CENTER
        ...
        for idx, row in enumerate(df.iterrows(), start=1):
            ...
            # Get data from column index 9 (same as line 2454)
            affected_system_data = str(df.iloc[index, 9]) if pd.notna(df.iloc[index, 9]) and str(df.iloc[index, 9]).strip() != '' else "NA"
            if affected_system_data == "NA" or affected_system_data.strip() == '':
                affected_system_data = "No affected systems"
            replace_affected_systems_with_table(new_table, "Affected_URL___", affected_system_data)
...
def read_scope_worksheet_data(excel_file):
    ...
    try:
        ...
        # Read data from column B only (index 1) starting from row 2
        for row in ws.iter_rows(min_row=2, values_only=True):
            # Check if row has at least column B (index 1)
            if len(row) < 2:
                continue
            
            # Get data from column B (index 1) - this is the Host column
            host_value = str(row[1]).strip() if row[1] else ""
            
            # If we have data in column B, add it to scope_data
            if host_value:
                scope_data.append({
                    'host': host_value
                })
...
def replace_scope_placeholders_with_data(doc, excel_file):
    """
    Find and replace 'UUUUURRLLL' placeholder with data from the Scope worksheet,
    creating new rows for each entry
    """
    try:
        ...
        # Find tables containing the UUUUURRLLL placeholder
        for table in doc.tables:
            for row_idx, row in enumerate(table.rows):
                for cell_idx, cell in enumerate(row.cells):
                    for paragraph in cell.paragraphs:
                        cell_text = paragraph.text
                        if "UUUUURRLLL" in cell_text:
                            print(f"🔍 Found UUUUURRLLL placeholder in row {row_idx}, cell {cell_idx}: {cell_text}")
                            # Found the target row, now replace and add new rows
                            replace_scope_placeholders_in_table(table, row_idx, cell_idx, scope_data)
                            return
        
        print("⚠️ UUUUURRLLL placeholder not found in any table")
...
def replace_scope_placeholders_in_table(table, target_row_idx, target_cell_idx, scope_data):
    ...
    try:
        ...
        # Process scope data - just use host values from column B
        processed_data = []
        for entry in scope_data:
            host = entry.get('host', '')
            if host:  # Only add if host exists
        ...
        if processed_data:
            ...
            # Replace UUUUURRLLL with host
            for paragraph in target_row.cells[target_cell_idx].paragraphs:
                if "UUUUURRLLL" in paragraph.text:
                    _replace_text_in_runs(paragraph, "UUUUURRLLL", first_entry['host'])
        ...
        for i in range(1, len(processed_data)):
            ...
            for j, cell in enumerate(new_row.cells):
                if j < len(target_row.cells):
                    ...
                    if j == target_cell_idx:  # UUUUURRLLL cell - replace with host
                        new_content = entry['host']
...
def replace_a11_row_with_vulnerabilities(doc, excel_file):
    ...
    try:
        ...
        if not df.empty and len(df) >= 1:
            ...
            note_message = "Note: No vulnerabilities were identified during the Website VAPT Audit."
        ...
        # If A2 is empty, delete the a.1.1 table and its page
        if a2_empty:
            print("A2 is empty - removing a.1.1 table and its page...")
            # Remove the entire table
            target_table._element.getparent().remove(target_table._element)
            print("✅ Removed a.1.1 table")
...
def generate_word_report_from_excel(excel_file_path):
    ...
    try:
        ...
        template_path = os.path.join('static', 'Formats_and_Catalog', 'Website_VAPT_First_Audit_Report.docx')
...
@website_vapt_word_report_bp.route('/process_website_vapt_first_audit_word_report', methods=['POST'])
def process_website_first_audit_word_report():
    """
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                word_temp_file_path,
                as_attachment=True,
                download_name='Website_VAPT_First_Audit_Report.docx',
        ...
        logger.error(f"Error generating website first audit word report: {type(e).__name__}: {str(e)}", exc_info=True)

# Web Application VAPT Follow-Up Audit Excel Report Generator
...
# Create a Blueprint for Web Application VAPT Follow-Up routes
web_app_vapt_followup_bp = Blueprint('web_app_vapt_followup', __name__)

def generate_web_app_followup_filename(organization, end_date):
    """
    Generate static filename for Web Application VAPT Follow-Up Audit.
    Format: Web_Application_VAPT_Follow_Up_Audit
    """
    return "Web_Application_VAPT_Follow_Up_Audit_Report.xlsx"
...
def find_latest_status_column_in_user_excel(user_excel_path, ws_name="Web Application VAPT"):
    ...
    try:
        ...
        for sheet_name in user_wb.sheetnames:
            if ws_name.lower() in sheet_name.lower() or "web application" in sheet_name.lower():
...
def extract_poc_images_from_user_excel(user_excel_path, ws_name="Web Application VAPT"):
    ...
    try:
        ...
        temp_dir = f"Temp_POC_WebApp_{timestamp}"
...
def create_web_app_followup_metadata_worksheet(wb, form_data, user_excel_path=None):
...
@web_app_vapt_followup_bp.route('/process_web_app_vapt_follow_up_audit_report', methods=['POST'])
def process_web_app_follow_up_audit_report():
    """Main endpoint to process Web Application VAPT Follow-Up Audit Report."""
    temp_files = []
    temp_dirs = []
    
    try:
        print("\n" + "="*80)
        print("🚀 Processing Web Application VAPT Follow-Up Audit Report")
        print("="*80)
        ...
        user_ws_name = None
        for sheet_name in user_wb.sheetnames:
            if 'web application vapt' in sheet_name.lower() or 'website_vapt' in sheet_name.lower():
        ...
        # Copy "Web Application VAPT" worksheet from user Excel
        copy_success, temp_image_files = copy_worksheet_with_images(user_wb, user_ws_name, wb, "Web Application VAPT")
        if not copy_success:
            user_wb.close()
        ...
        if user_status_col:
            try:
                ...
                user_ws = None
                for sheet_name in user_wb.sheetnames:
                    if user_ws_name.lower() in sheet_name.lower() or "web application" in sheet_name.lower():
        ...
        create_web_app_followup_metadata_worksheet(wb, form_data, user_excel_path)
        ...
        filename = generate_web_app_followup_filename('', form_data.get('endDate', ''))

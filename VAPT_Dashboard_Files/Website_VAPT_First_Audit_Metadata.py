...
# Create blueprint for Website VAPT First Audit MetaData
website_vapt_first_audit_metadata_bp = Blueprint('website_vapt_first_audit_metadata', __name__)

def create_website_first_audit_metadata_excel(form_data):
    ...
    try:
        ...
        if vulnerability_status == '0':
            ...
            note_text = "Note: No vulnerabilities were identified during the first Website VAPT audit."
        ...
        filename = 'Website_First_Audit_MetaData.xlsx'
...
@website_vapt_first_audit_metadata_bp.route('/process_website_vapt_first_audit_metadata', methods=['POST'])
def process_website_vapt_first_audit_metadata():
    """Process Website VAPT First Audit MetaData form submission"""
    try:
        print("\n" + "="*80)
        ...
        # Create Excel file
        filepath, filename = create_website_first_audit_metadata_excel(form_data)
...
# Cleanup endpoint
@website_vapt_first_audit_metadata_bp.route('/cleanup_website_vapt_first_audit_metadata', methods=['POST'])
def cleanup_website_vapt_first_audit_metadata():
    """Clean up old Website VAPT First Audit MetaData Excel files"""
    try:
        uploads_dir = os.path.join('static', 'uploads')
        if os.path.exists(uploads_dir):
            target_file = 'Website_First_Audit_MetaData.xlsx'

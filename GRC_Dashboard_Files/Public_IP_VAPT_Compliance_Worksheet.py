...
# Create blueprint for Public IP VAPT Compliance
grc_public_ip_vapt_compliance_bp = Blueprint('grc_public_ip_vapt_compliance', __name__)

def create_public_ip_vapt_compliance_excel(form_data, excel_file_path):
    """Create Public IP VAPT Compliance Excel file"""
    try:
        print(f"🔍 DEBUG: Starting Public IP VAPT Compliance Excel creation...")
        ...
        ws['B2'] = f"Compliance Of Public IP VAPT Audit Report (Report ID no. {report_id} dated {report_date_formatted})"
        ...
        # Find the "Public_IP_VAPT" worksheet
        source_ws = None
        for sheet_name in source_wb.sheetnames:
            if sheet_name == "Public_IP_VAPT":
                source_ws = source_wb[sheet_name]
                break
        
        if not source_ws:
            raise Exception("Worksheet 'Public_IP_VAPT' not found in uploaded Excel file")
        
        print(f"✅ Found 'Public_IP_VAPT' worksheet with {source_ws.max_row} rows")
        ...
        print(f"✅ Public IP VAPT Compliance Excel created successfully")
        
        return wb
        
    except Exception as e:
        print(f"❌ Error creating Public IP VAPT Compliance Excel: {e}")
        import traceback
        traceback.print_exc()
        raise

@grc_public_ip_vapt_compliance_bp.route('/grc_process_public_ip_vapt_compliance', methods=['POST'])
def process_public_ip_vapt_compliance():
    """Process Public IP VAPT Compliance form submission"""
    try:
        print("\n" + "="*80)
        print("🚀 Processing Public IP VAPT Compliance")
        print("="*80)
        ...
        excel_path = os.path.join(upload_dir, f'temp_public_ip_vapt_{datetime.now().strftime("%Y%m%d_%H%M%S")}.xlsx')
        
        excel_file.save(excel_path)
        
        # Create the compliance Excel
        wb = create_public_ip_vapt_compliance_excel(form_data, excel_path)
        
        # Save the final workbook
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        filename = f'Public_IP_VAPT_Compliance_{timestamp}.xlsx'
        ...
        print(f"\n✅ Public IP VAPT Compliance Excel created: {filename}")
        print("="*80)
...
@grc_public_ip_vapt_compliance_bp.route('/grc_cleanup_public_ip_vapt_compliance', methods=['POST'])
def cleanup_public_ip_vapt_compliance():
    """Clean up Public IP VAPT Compliance Excel file after download"""
    try:
        ...
        if os.path.exists(temp_dir):
            try:
                temp_files = [f for f in os.listdir(temp_dir) if f.startswith('temp_public_ip_vapt_')]

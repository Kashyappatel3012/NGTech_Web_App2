...
# Create blueprint for Infrastructure VAPT Compliance
grc_infra_vapt_compliance_bp = Blueprint('grc_infra_vapt_compliance', __name__)

def create_infra_vapt_compliance_excel(form_data, excel_file_path):
    """Create Infrastructure VAPT Compliance Excel file"""
    try:
        print(f"🔍 DEBUG: Starting Infrastructure VAPT Compliance Excel creation...")
        ...
        for sheet_name in source_wb.sheetnames:
            if sheet_name == "Infra_VAPT":
                source_ws = source_wb[sheet_name]
                break
        
        if not source_ws:
            raise Exception("Worksheet 'Infra_VAPT' not found in uploaded Excel file")
        
        print(f"✅ Found 'Infra_VAPT' worksheet with {source_ws.max_row} rows")
        ...
        print(f"✅ Infrastructure VAPT Compliance Excel created successfully")
        
        return wb
        
    except Exception as e:
        print(f"❌ Error creating Infrastructure VAPT Compliance Excel: {e}")
        import traceback
        traceback.print_exc()
        raise

@grc_infra_vapt_compliance_bp.route('/grc_process_infra_vapt_compliance', methods=['POST'])
def process_infra_vapt_compliance():
    """Process Infrastructure VAPT Compliance form submission"""
    try:
        print("\n" + "="*80)
        print("🚀 Processing Infrastructure VAPT Compliance")
        print("="*80)
        ...
        excel_path = os.path.join(upload_dir, f'temp_infra_vapt_{datetime.now().strftime("%Y%m%d_%H%M%S")}.xlsx')
        
        excel_file.save(excel_path)
        
        # Create the compliance Excel
        wb = create_infra_vapt_compliance_excel(form_data, excel_path)
        
        # Save the final workbook
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        filename = f'Infrastructure_VAPT_Compliance_{timestamp}.xlsx'
        ...
        print(f"\n✅ Infrastructure VAPT Compliance Excel created: {filename}")
        print("="*80)
...
@grc_infra_vapt_compliance_bp.route('/grc_cleanup_infra_vapt_compliance', methods=['POST'])
def cleanup_infra_vapt_compliance():
    """Clean up Infrastructure VAPT Compliance Excel file after download"""
    try:
        ...
        if os.path.exists(temp_dir):
            try:
                temp_files = [f for f in os.listdir(temp_dir) if f.startswith('temp_infra_vapt_')]

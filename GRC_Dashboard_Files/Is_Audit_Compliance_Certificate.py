...
# Create blueprint for GRC IS Audit Compliance Certificate
grc_is_audit_compliance_certificate_bp = Blueprint('grc_is_audit_compliance_certificate', __name__)
...
def create_is_audit_compliance_certificate(form_data):
    """Create IS Audit Compliance Certificate Word document"""
    try:
        print(f"🔍 Starting IS Audit Compliance Certificate creation...")
        ...
        # Load template
        template_path = os.path.join('static', 'Formats_and_Catalog', 'IS Audit Compliance Certificate.docx')
        ...
        print(f"   ✅ IS Audit Compliance Certificate created successfully")
...
@grc_is_audit_compliance_certificate_bp.route('/grc_process_is_audit_compliance_certificate', methods=['POST'])
def process_is_audit_compliance_certificate():
    """Process IS Audit Compliance Certificate form submission"""
    try:
        print("\n" + "="*80)
        print("🚀 Processing IS Audit Compliance Certificate")
        print("="*80)
        ...
        doc = create_is_audit_compliance_certificate(form_data)
        
        # Save the Word document
        filename = 'IS_Audit_Compliance_Certificate.docx'
        filepath = os.path.join('static', 'uploads', filename)
        
        os.makedirs(os.path.dirname(filepath), exist_ok=True)
        doc.save(filepath)
        
        print(f"\n✅ IS Audit Compliance Certificate created: {filename}")
        print("="*80)
...
@grc_is_audit_compliance_certificate_bp.route('/grc_cleanup_is_audit_compliance_certificate', methods=['POST'])
def cleanup_is_audit_compliance_certificate():
    """Clean up IS Audit Compliance Certificate file after download"""
    try:
